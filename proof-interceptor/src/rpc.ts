...
function validateCheckTransaction(
  ...
  if (kind.data.type !== "INVOKE") {
    return unsupportedTxVersion(
      requestId,
      `Only INVOKE transactions are supported, got: ${String(kind.data.type)}`
    );
  }

  if (kind.data.version !== "0x3") {
    return unsupportedTxVersion(
      requestId,
      `Only version 0x3 is supported, got: ${String(kind.data.version)}`
    );
  }
