...
pub(crate) impl UserImpl of UserTrait {
    ...
    fn approve(self: @User, token: Token, amount: u256, spender: ContractAddress) {
        let token_addr = token.contract_address();
        cheat_caller_address_once(contract_address: token_addr, caller_address: *self.address);
        IERC20Dispatcher { contract_address: token_addr }.approve(:spender, :amount);
    }

    fn deposit(self: @User, token: Token, amount: u128, note: EncNote) {
        self.approve(:token, amount: amount.into(), spender: *self.server);
        IServerDispatcher { contract_address: *self.server }
            .deposit(owner: *self.address, token: token.contract_address(), :amount, :note);
    }
