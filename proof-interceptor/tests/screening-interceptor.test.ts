...
describe("ScreeningInterceptor", () => {
  ...
  describe("screening subjects beyond the depositor", () => {
    /** The client action creating the open note the invoke then funds. */
    ...
    /** A `ComputeAndInvoke` on the anonymizer settling one open note, i.e. one shadow account. */
    function anonymizerInteraction(): CairoCustomEnum {
      return new CairoCustomEnum({
        ComputeAndInvoke: {
          contract_address: ANONYMIZER_ADDR,
          compute_additional_data: [0x646170n, 7n],
          invoke_additional_data: [
            "0x0", // no dapp calls
            "0x1", // one open note
            "0x3", // note_id
            TOKEN,
            "0x0", // CollectPolicy::All
          ],
        },
      });
    }
    ...
    it("blocks when an invoke target's policy cannot be read", async () => {
      ...
      const invoke = new CairoCustomEnum({
        InvokeExternal: { contract_address: "0xe0e0", calldata: ["0x1"] },
      });

      const verdict = await interceptor.intercept(
        poolCallTransaction([createOpenNoteAction(), invoke])
      );
