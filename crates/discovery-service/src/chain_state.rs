...
pub trait ChainState: Send + Sync {
    ...
    /// - `Ok(false)`: the node has no such block, has reported it orphaned, or
    ///   now carries a different block at that height. A height that resolves
    ///   to a pre-confirmed block also reports `false`, since a pre-confirmed
    ///   block carries no hash and so can never be the one asked about.
