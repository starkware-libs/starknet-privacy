import type { AccountInterface, AllowArray, BigNumberish, BlockIdentifier, BlockNumber, Call, Invocation } from "starknet";
...
export interface PrivateTransfers {
   ...
   */
  setupRequirement(recipient: PrivateRecipient, token: StarknetAddress): Promise<{register: boolean,initial: boolean, token: boolean}>;

  /**
   * if an intended recipient doesn't have a Channel associated with it
   */
  setupInitial(recipient: StarknetAddress): Promise<{invocationData: CallAndProof, channel: Channel}>;

  /**
   * if an intended recipient doesn't have the token in its channel. TBD: expose tokens in the channel. 
   */
  setupToken(recipient: PrivateRecipient, token: StarknetAddress): Promise<{invocationData: CallAndProof, channel: Channel}>;

  /**
   * deposit tokens into the privacy pool
   * 
   * v1: the recipient is the same as the account address
   */
  deposit(recipient: PrivateRecipient, token: StarknetAddress, amount: bigint, selfChannel?: Channel): Promise<PrivateInvocationResult>;
   ...
   *       {recipient: Self, token: STRK, amount: 10n, channel: selfChannel}
   *     ],
   *     call: {
  ...
  composite(
    ...
    outputs: {recipient: PrivateRecipient | StarknetAddress, token: StarknetAddress, amount: bigint, channel: Channel | "withdraw"}[],
    call?: Call,
   ...
   */
  discover(since?: BlockIdentifier, known?: PrivacyState): Promise<PrivacyState>;
...
export interface DiscoveryProviderInterface {
   ...
   */
  discover(address: StarknetAddress, viewingKey: ViewingKey, since?: BlockIdentifier, known?: PrivacyState): Promise<PrivacyState>;
