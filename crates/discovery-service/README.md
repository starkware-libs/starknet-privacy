...
Note: Integration tests spawn starknet-devnet instances and must run sequentially to avoid port conflicts.
...
```bash
cd sdk && \
DUMP_DEVNET_PATH=../crates/discovery-service/tests/fixtures/devnet-dump.json.gz \
