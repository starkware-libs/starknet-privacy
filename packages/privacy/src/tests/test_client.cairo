...
use privacy::tests::utils_for_tests::{
    ...
    EchoExecutorCfgTrait, InvokeExternalInputIntoServerActionTrait, NoteZero, PrivacyCfgTrait, Test,
    TestTrait, UserTrait, constants, decrypt_channel_info, decrypt_outgoing_channel_info,
...
fn test_create_note_twice() {
    ...
    // Note 3: open note at index 2 (enc → open). Create+fill via echo executor.
    let note_3 = user_1
        .new_open_note_with_generated_random(
            recipient: user_2, :token_addr, index: 2, depositor: test.privacy.echo_executor.address,
        );
    let note_id_3 = user_1
        .create_and_fill_open_note_e2e(
            executor: @test.privacy.echo_executor,
            create_note_input: note_3,
            amount: open_note_amount,
            :token,
        );

    // Note 4: open note at index 3 (open → open). Create+fill via echo executor.
    let note_4 = user_1
        .new_open_note_with_generated_random(
            recipient: user_2, :token_addr, index: 3, depositor: test.privacy.echo_executor.address,
        );
    let note_id_4 = user_1
        .create_and_fill_open_note_e2e(
            executor: @test.privacy.echo_executor,
            create_note_input: note_4,
            amount: open_note_amount,
            :token,
        );
    ...
    // Verify filled open notes have the same packed_value (same salt + amount).
...
fn test_create_open_note_stores_correctly() {
    ...
    let create_note_input = user_1
        .new_open_note_with_generated_random(
            recipient: user_2, :token_addr, :index, depositor: test.privacy.echo_executor.address,
        );
    // Create and fill the open note.
    let note_id = user_1
        .create_and_fill_open_note_e2e(
            executor: @test.privacy.echo_executor, :create_note_input, :amount, :token,
        );
    ...
    assert_eq!(stored_note.depositor, test.privacy.echo_executor.address);
...
// TODO: Remove? Should no longer be possible (and we're counting on it) to create an open note
// without filling it.
#[test]
...
fn test_use_deposited_open_note(open_note_self: bool) {
    ...
    user_1.cheat_create_filled_open_note_in_storage(:create_note_input, :amount);
...
fn test_use_multiple_deposited_open_notes() {
    ...
    let create_note_input_1 = user_1
        .new_open_note_with_generated_random(
            recipient: user_2, :token_addr, index: 0, depositor: test.privacy.echo_executor.address,
        );
    let create_note_input_2 = user_1
        .new_open_note_with_generated_random(
    ...
    user_1
        .cheat_create_filled_open_note_in_storage(
            create_note_input: create_note_input_1, amount: amount_1,
        );
    user_1
        .cheat_create_filled_open_note_in_storage(
            create_note_input: create_note_input_2, amount: amount_2,
        );
    test.privacy.increase_token_balance(:token, amount: amount_1);
    test.privacy.increase_token_balance(:token, amount: amount_2);
...
fn test_use_mixed_open_and_enc_notes() {
    ...
    user_1
        .cheat_create_filled_open_note_in_storage(
            create_note_input: open_note_input, amount: open_amount,
        );
    test.privacy.increase_token_balance(:token, amount: open_amount);
...
fn test_create_open_note_decrypt_recipient_addr() {
    ...
    let amount = constants::DEFAULT_AMOUNT;
    user_1.open_channel_with_token_e2e(recipient: user_2, :token_addr, outgoing_channel_index: 0);
    // Create an open note (with fill, so the enforcement passes and events are emitted).
    let create_note_input = user_1
        .new_open_note_with_generated_random(
            recipient: user_2, :token_addr, index: 0, depositor: depositor,
        );
    let mut spy_events = spy_events();
    user_1
        .create_and_fill_open_note_e2e(
...
fn test_execute_deposit_swap() {
    ...
    user.cheat_create_open_note_in_storage(create_note_input: create_open_note_input);
...
fn test_internal_actions() {
    ...
    let mut create_open_note_input = user_1
        .new_open_note_with_generated_random(
            recipient: user_2, :token_addr, :index, depositor: depositor.address,
        );
    ...
    user_1
        .cheat_create_filled_open_note_in_storage(
            create_note_input: create_open_note_input, :amount,
        );
    test.privacy.increase_token_balance(:token, :amount);
    let nullifier = user_2.compute_nullifier(sender: user_1, :token_addr, :index);
...
fn test_execute_create_open_note() {
    ...
    let create_note_input = user_1
        .new_open_note(
            ...
            :token_addr,
            :index,
            depositor: test.privacy.echo_executor.address,
            :random,
        );
    ...
    token.supply(address: test.privacy.echo_executor.address, :amount);
    token
        .approve(
            owner: test.privacy.echo_executor.address,
    ...
    let expected_filled_event = events::OpenNoteDeposited {
        depositor: test.privacy.echo_executor.address, token: token_addr, note_id, amount,
    };
    assert_expected_event_emitted(
        spied_event: events[1],
        expected_event: expected_filled_event,
        expected_event_selector: @selector!("OpenNoteDeposited"),
        expected_event_name: "OpenNoteDeposited",
    );
}

// TODO: Keep refactor (and rename) or remove? Can no longer be used as a single action.
#[test]
fn test_create_open_note_as_single_action() {
    ...
    let create_note_input = user
        .new_open_note_with_generated_random(
            recipient: user, :token_addr, index: 0, depositor: test.privacy.echo_executor.address,
        );
    let (note_id, expected_note) = user.compute_open_note_with_amount(:create_note_input, :amount);
    ...
    // CreateOpenNote + InvokeExternal - should succeed (CreateOpenNote is a privacy action).
    let client_actions = [
        ClientAction::CreateOpenNote(create_note_input), ClientAction::InvokeExternal(invoke_input),
    ]
        .span();
...
fn test_create_open_and_enc_notes_same_tx() {
    ...
    // Pre-compute note IDs and expected filled notes.
    ...
    token.supply(address: test.privacy.echo_executor.address, amount: 2 * amount);
    token
        .approve(
            owner: test.privacy.echo_executor.address,
            spender: test.privacy.address,
...
fn test_create_note_at_existing_note_id(initial_is_open: bool, colliding_is_open: bool) {
    ...
    if initial_is_open {
        user_1
            .cheat_create_filled_open_note_in_storage(create_note_input: open_note_input, :amount);
    } else {
        user_1.cheat_create_enc_note_e2e(create_note_input: enc_note_input);
    }
    ...
    // Use the initial note (increase balance if open, then spend).
    if initial_is_open {
        test.privacy.increase_token_balance(:token, :amount);
    }
    let channel_key = user_1.compute_channel_key(recipient: user_2);
...
fn test_deposit_to_open_note_twice() {
    // Test that depositing to an already-filled open note fails with NOTE_ALREADY_DEPOSITED.
    let mut test: Test = Default::default();
    let mut user_1 = test.new_user();
    let mut user_2 = test.new_user();
    let mut user_3 = test.new_user();
    ...
    // TX 1: Create and fill open note A in the same transaction.
    ...
    let note_id_a = user_1
        .create_and_fill_open_note_e2e(
            executor: @test.privacy.echo_executor,
            create_note_input: create_note_input_a,
            :amount,
            :token,
        );

    // TX 2: Create note B and attempt to deposit into the already-filled note A.
    let create_note_input_b = user_1
        .new_open_note_with_generated_random(
            recipient: user_2, :token_addr, index: 1, depositor: test.privacy.echo_executor.address,
        );
    let create_actions_b = create_note_input_b.into_server_actions(user: user_1);
    token.supply(address: test.privacy.echo_executor.address, :amount);
    token
        .approve(
            owner: test.privacy.echo_executor.address,
    ...
    let bad_deposit = test
        .privacy
        ._deposit_to_open_note_actions(note_id: note_id_a, :token_addr, :amount);
    let mut actions: Array<ServerAction> = create_actions_b.into();
    actions.append_span(bad_deposit);
