...
fn test_apply_actions_rejects_multiple_depositors() {
    ...
    let token_addr = token.contract_address();
    let echo_executor = test.privacy.echo_executor;
    // A second echo executor at a distinct address stands in for a different depositor.
    let other_echo_executor = deploy_mock_echo_with_salt(salt: 1);
    ...
    let (note_id_second, _) = user.compute_open_note(create_note_input: create_input_second);

    // Fund both executors so each deposit would succeed in isolation: only the differing
    // depositor identity should make `apply_actions` revert.
    token.supply(address: echo_executor, :amount);
    token.approve(owner: echo_executor, spender: test.privacy.address, amount: amount.into());
    token.supply(address: other_echo_executor, :amount);
    token.approve(owner: other_echo_executor, spender: test.privacy.address, amount: amount.into());

    // The client compiler permits only one InvokeExternal per tx, so two depositors can only
    // arise server-side. Assemble the ServerActions directly: create+deposit note one via
    // `echo_executor`, then create+deposit note two via `other_echo_executor`.
    let deposit_first = OpenNoteDeposit { note_id: note_id_first, token: token_addr, amount };
    let deposit_second = OpenNoteDeposit { note_id: note_id_second, token: token_addr, amount };
    let mut actions: Array<ServerAction> = create_input_first.into_server_actions(:user).into();
    actions
        .append_span(
            test
                .privacy
                .invoke_external_echo_deposits([deposit_first].span())
                .into_server_actions(),
        );
    ...
    actions
        .append_span(
            test
                .privacy
                .invoke_external_echo_deposits_to(
                    executor: other_echo_executor, deposits: [deposit_second].span(),
                )
                .into_server_actions(),
        );

    let result = test.privacy.safe_apply_actions(actions: actions.span());
    assert_panic_with_felt_error(:result, expected_error: errors::MULTIPLE_DEPOSITORS);
}
