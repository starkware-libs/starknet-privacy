...
/// Every `OpenNoteScreeningPolicy`, in declaration order, so a test sweeping the policies cannot
/// silently miss a variant added later. Declaration order is load-bearing for
/// `test_screening_policy_store_index_matches_serde_index`, which pairs this list with the
