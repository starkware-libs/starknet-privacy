...
/// Result of a batched exponential probe (non-empty subchannel).
///
/// Returned only when the probe finds at least one note and a sentinel.
/// Empty subchannels return `None`; config-exceeded cases return `Err`.
/// Probe hits are inserted into the caller-provided cache.
#[derive(Debug)]
struct ExponentialProbeResult {
    /// Last index confirmed to exist (bisection lower bound).
    lower_bound: u64,
    /// First index confirmed empty (bisection upper bound).
    upper_bound: u64,
}
...
pub fn boundary_budget(max_note_log_index: u32) -> usize {
    (2 * max_note_log_index as usize + 1) * COST_NOTE_PROBING
}
...
pub async fn find_last_note_index<S: IViews>(
    ...
    let max_bisect_steps = max_note_log_index as usize;
    let mut cache = HashMap::new();

    let Some(result) = exponential_probe(
        pool,
        channel_key,
        token,
        start_index,
...
/// the exact last occupied index. Hits are inserted into `cache`.
///
...
///
/// Returns:
/// - `Ok(None)` — empty subchannel (first probe missed).
/// - `Ok(Some(result))` — bounds found, ready for bisection.
...
async fn exponential_probe<S: IViews>(
    ...
    max_note_log_index: u32,
    cache: &mut HashMap<u64, (Felt, Felt)>,
) -> Result<Option<ExponentialProbeResult>, DiscoveryError> {
    ...
    for (i, &packed) in results.iter().enumerate() {
        ...
        if packed != Felt::ZERO {
            ...
            return Ok(lower_bound.map(|lower_bound| ExponentialProbeResult {
                lower_bound,
                upper_bound: idx,
            }));
        }
...
mod tests {
    ...
    async fn test_exponential_probe_multiple_elements() {
        ...
        assert_eq!(result.upper_bound, 7);
