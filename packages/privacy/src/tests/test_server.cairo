...
use privacy::tests::utils_for_tests::{
    ...
    _deploy_mock_echo_with_salt, constants, deploy_mock_reentrancy, deploy_mock_return_garbage,
...
fn test_deposit_to_open_note_paused() {
    ...
    let result = test
        ...
        .safe_deposit_to_open_note(
            executor: test.privacy.echo_executor, note_id: 1, :token_addr, amount: 1,
        );
    assert_panic_with_felt_error(:result, expected_error: PausableErrors::PAUSED);
}
...
fn test_deposit_to_open_note() {
    ...
    // Execute _deposit_to_open_note via the echo executor.
    test
        .privacy
        ._deposit_to_open_note(executor: echo_executor_addr, :note_id, :token_addr, :amount);
...
fn test_deposit_to_open_note_assertions() {
    ...
    let result = test
        .privacy
        .safe_deposit_to_open_note(
            executor: echo_executor_addr, note_id: note_id_enc, :token_addr, :amount,
        );
    ...
    // executor.
    let wrong_echo_executor = _deploy_mock_echo_with_salt(salt: 1);
    let (note_id, _) = test.mock_new_note(:amount);
    let note = open_note(token: token_addr, depositor: echo_executor_addr);
    test.privacy.cheat_create_note(:note_id, :note);
...
fn test_deposit_to_open_note_transfer_assertions() {
    ...
    let result = test
        .privacy
        .safe_deposit_to_open_note(executor: echo_executor_addr, :note_id, :token_addr, :amount);
    assert_panic_with_error(:result, expected_error: Erc20Error::INSUFFICIENT_BALANCE.describe());
