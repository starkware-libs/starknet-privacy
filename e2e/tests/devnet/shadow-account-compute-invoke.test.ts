...
describe("shadow account anonymizer compute-and-invoke on devnet", () => {
  ...
  beforeAll(async () => {
    ...
    env.screening.anonymizerAddress = shadowAccount.anonymizer;
  ...
  it("pool rejects the collection when the prover attests no shadow account", async () => {
    ...
    // The SDK's mock prover attests only plain deposits, and this transaction has none — it
    // produces no attestation, while the pool expects one for the shadow account.
    const mockProverAlice = createPrivateTransfers({
      account: de.alice,
    ...
    await expect(
      provePayoutCollection(mockProverAlice, 1n, shadowAccount).then(
  ...
  it("collects through an unlisted anonymizer by attesting the anonymizer itself", async () => {
    ...
    // attestation naming the anonymizer itself. `env.screening.anonymizerAddress` still points at
    // the delegated one — the config only steers `Delegated` derivation, not this path.
