...
pub(crate) impl UserImpl of UserTrait {
    ...
    fn internal_open_subchannel_with_new_salt(
    ...
    fn open_subchannel_e2e(
        ...
        let salt = self.get_salt().into();
        let mut spy = spy_messages_to_l1();
        self.open_subchannel(:recipient, :token_address, :index, :salt);
        let actions = spy_messages_to_server_actions(ref :spy);
        self.privacy.revert_actions_for_testing(:actions);
        self.privacy.server.execute_actions(:actions);
        salt
    ...
    fn open_channel_with_token_e2e(
        ...
        (random_channel, salt_subchannel)
    }

    /// Returns a random value.
    fn get_random(ref self: User) -> felt252 {
        let salt = self.get_salt();
        hash(['RANDOM', salt.into()].span())
    }

    fn get_salt(ref self: User) -> u32 {
        self.salt += 1;
        self.salt
    }
    ...
    fn compute_enc_subchannel_info(
        self: @User, recipient: User, token_address: ContractAddress, salt: felt252,
    ) -> EncSubchannelInfo {
        let channel_key = self.compute_channel_key(:recipient);
        encrypt_subchannel_info(:channel_key, token: token_address, :salt)
    }
...
pub(crate) impl TestImpl of TestTrait {
    fn new_user(ref self: Test) -> User {
        self.salt += 1;
        let mut private_key = 'PRIVATE_KEY' + self.salt.into();
        if !is_canonical_key(key: private_key) {
            private_key = Neg::neg(private_key);
        }
        let public_key = derive_public_key(:private_key);
        self.salt += 1;
        let address = deploy_mock_account(salt: self.salt.into());
        User { address, privacy: self.privacy, private_key, public_key, salt: Zero::zero() }
    }

    /// Mock function to generate a new token address.
    fn mock_new_token(ref self: Test) -> ContractAddress {
        self.salt += 1;
        ('TOKEN_ADDRESS' + self.salt.into()).try_into().unwrap()
    }
    ...
    fn mock_new_enc_private_key(ref self: Test) -> EncPrivateKey {
        self.salt += 1;
        EncPrivateKey {
            ephemeral_pubkey: 'EPHEMERAL_PUBKEY' + self.salt.into(),
            enc_private_key: 'ENC_PRIVATE_KEY' + self.salt.into(),
        }
    }
    ...
    fn mock_new_channel(ref self: Test) -> (EncChannelInfo, felt252) {
        ...
        let enc_channel_info = EncChannelInfo {
            ephemeral_pubkey: 'EPHEMERAL_PUBKEY' + self.salt.into(),
            enc_channel_key: 'ENC_CHANNEL_KEY' + self.salt.into(),
            enc_sender_addr: 'ENC_SENDER_ADDR' + self.salt.into(),
        };
        let channel_id = 'CHANNEL_ID' + self.salt.into();
    ...
    fn mock_new_subchannel(ref self: Test) -> (felt252, felt252, EncSubchannelInfo) {
        self.salt += 1;
        let subchannel_id = 'SUBCHANNEL_ID' + self.salt.into();
        let subchannel_key = 'SUBCHANNEL_KEY' + self.salt.into();
        let enc_subchannel_info = EncSubchannelInfo {
            salt: self.salt.into(), enc_token: 'ENC_TOKEN' + self.salt.into(),
        };
    ...
    fn mock_new_note(ref self: Test, amount: u128) -> EncNote {
        self.salt += 1;
        let id = 'NOTE_ID' + self.salt.into();
        let enc_amount = 'ENC_AMOUNT' + amount.into() + self.salt.into();
    ...
    fn mock_new_nullifier(ref self: Test) -> felt252 {
        self.salt += 1;
        'NULLIFIER' + self.salt.into()
    }

    fn new_token(ref self: Test) -> Token {
        self.salt += 1;
        let config = TokenConfig {
            name: format!("Token {}", self.salt),
            symbol: format!("Token {}", self.salt),
...
impl DefaultTestImpl of Default<Test> {
    fn default() -> Test {
        ...
        Test { privacy, salt: Zero::zero(), compliance_private_key, compliance_public_key }
    }
}
...
pub(crate) fn decrypt_subchannel_token(
    ...
    let token = enc_subchannel_info.enc_token
        - compute_enc_token_hash(:channel_key, salt: enc_subchannel_info.salt);
