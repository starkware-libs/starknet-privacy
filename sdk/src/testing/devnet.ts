...
export async function createDevnetTestEnv(
  ...
  // The in-repo pool screens deposits, so use a proving provider that signs
  // each deposit's attestation with the screener key the pool was deployed
  // with.
  const transfers = {
...
/**
 * A transfers object for `env.alice` whose proving provider never produces a
 * screening attestation, so a deposit goes out with a trailing
 * `Option::None`. The in-repo screening pool requires an attestation for
 * every deposit, so such a deposit is expected to revert; this is used to
 * assert an un-attested deposit cannot slip through. It shares alice's
