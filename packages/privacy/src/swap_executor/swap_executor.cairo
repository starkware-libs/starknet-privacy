...
pub mod SwapExecutor {
    ...
    pub impl SwapExecutorImpl of ISwapExecutor<ContractState> {
        fn swap(
            ...
            // Validate all inputs are non-zero/non-empty.
            ...
            let received_amount: u128 = (out_erc20.balance_of(account: self_address)
                - balance_before)
                .try_into()
                // TODO: Consider removing error, ask Lior.
                .expect(errors::RECEIVED_AMOUNT_OVERFLOW);

            // Approve caller to transfer received output funds.
            out_erc20.approve(spender: get_caller_address(), amount: received_amount.into());
