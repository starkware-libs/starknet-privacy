...
fn test_multiple_invoke_reverts() {
    ...
    let compute_invoke = ClientAction::ComputeAndInvoke(
        ComputeAndInvokeInput {
            contract_address: mock, compute_data: ['X'].span(), invoke_additional_data: [].span(),
        },
    );
...
fn test_compute_and_invoke_builds_invoke_with_computation() {
    ...
    // compute_data`, then forwards `compute_result ++ invoke_additional_data` (target preserved) as
    // the `InvokeWithComputation` server action. The mock echoes `identity_key` as the
    // compute_result.
    let mut test: Test = Default::default();
    let mut user = test.new_user();
    let random = user.get_random();
    ...
    let compute_and_invoke_input = ComputeAndInvokeInput {
        contract_address: mock, compute_data: ['PAYLOAD'].span(), invoke_additional_data,
    };
...
fn test_compute_and_invoke_missing_selector() {
    ...
    let compute_and_invoke_input = ComputeAndInvokeInput {
        contract_address: target, compute_data: ['X'].span(), invoke_additional_data: [].span(),
    };
    let client_actions = [
        ClientAction::SetViewingKey(SetViewingKeyInput { random }),
        ClientAction::ComputeAndInvoke(compute_and_invoke_input),
...
fn test_compute_and_invoke_rejects_empty_compute_result() {
    ...
    let compute_and_invoke_input = ComputeAndInvokeInput {
        contract_address: mock, compute_data: [].span(), invoke_additional_data: [].span(),
    };
    let client_actions = [
        ClientAction::SetViewingKey(SetViewingKeyInput { random }),
        ClientAction::ComputeAndInvoke(compute_and_invoke_input),
...
fn test_compute_and_invoke_assembles_empty_compute_and_multi_felt_result() {
    ...
    // `[identity_key]`) and a multi-felt `compute_result` (prepended ahead of
    ...
    let compute_and_invoke_input = ComputeAndInvokeInput {
        contract_address: mock, compute_data: [].span(), invoke_additional_data,
    };
    let client_actions = [
        ClientAction::SetViewingKey(SetViewingKeyInput { random }),
        ClientAction::ComputeAndInvoke(compute_and_invoke_input),
