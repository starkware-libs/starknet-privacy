...
fn testcheat_invoke_echo_paused() {
    ...
    let result = test
        .privacy
        .safe_invoke_echo(executor: test.echo_executor, note_id: 1, :token_addr, amount: 1);
    assert_panic_with_felt_error(:result, expected_error: PausableErrors::PAUSED);
}
...
#[test]
fn testcheat_invoke_echo() {
    let mut test: Test = Default::default();
    let token = test.new_token();
    let mut user = test.new_user();
    let amount = constants::DEFAULT_AMOUNT;
    let token_addr = token.contract_address();
    let echo_executor_addr = test.echo_executor;
    ...
    assert_eq!(token.balance_of(address: echo_executor_addr), amount.into());
    assert_eq!(token.balance_of(address: test.privacy.address), Zero::zero());
    ...
    test.privacy.cheat_invoke_echo(executor: echo_executor_addr, :note_id, :token_addr, :amount);
    ...
    assert_eq!(filled_note.depositor, echo_executor_addr);

    // Verify tokens transferred.
    assert_eq!(token.balance_of(address: echo_executor_addr), Zero::zero());
    assert_eq!(token.balance_of(address: test.privacy.address), amount.into());
...
#[test]
fn testcheat_invoke_echo_assertions() {
    let mut test: Test = Default::default();
    let token = test.new_token();
    let mut user = test.new_user();
    let amount = constants::DEFAULT_AMOUNT;
    ...
    let result = test
        .privacy
        .safe_invoke_echo(
            executor: echo_executor_addr, note_id: 1, token_addr: Zero::zero(), :amount,
        );
    ...
    let result = test
        .privacy
        .safe_invoke_echo(
            executor: echo_executor_addr, note_id: some_note_id, :token_addr, amount: 0,
        );
    ...
    let result = test
        .privacy
        .safe_invoke_echo(
            executor: echo_executor_addr, note_id: nonexistent_note_id, :token_addr, :amount,
        );
    ...
    let result = test
        .privacy
        .safe_invoke_echo(executor: echo_executor_addr, note_id: note_id_enc, :token_addr, :amount);
    ...
    test
        .privacy
        .cheat_invoke_echo(
            executor: echo_executor_addr, note_id: note_id_filled, :token_addr, :amount,
        );
    ...
    let result = test
        .privacy
        .safe_invoke_echo(
            executor: echo_executor_addr, :note_id, token_addr: test.mock_new_token(), :amount,
        );
...
#[test]
fn testcheat_invoke_echo_transfer_assertions() {
    let mut test: Test = Default::default();
    let token = test.new_token();
    let mut user = test.new_user();
    let amount = constants::DEFAULT_AMOUNT;
    ...
    let result = test
        .privacy
        .safe_invoke_echo(executor: echo_executor_addr, :note_id, :token_addr, :amount);
    assert_panic_with_error(:result, expected_error: Erc20Error::INSUFFICIENT_BALANCE.describe());
