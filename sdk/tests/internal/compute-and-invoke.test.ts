 ...
 * privacy_compute with the derived identity key + computeAdditionalData, then forwards the result +
 * invokeAdditionalData to privacy_invoke_with_computation.
 */
...
describe("ComputeAndInvoke", () => {
  ...
  it("simulate flow prepends identity key to computeAdditionalData and compute result to invokeAdditionalData", async () => {
    ...
    const computeAdditionalData = [7n, 8n];
    const invokeAdditionalData = [42n];
    ...
    expect(compute.computeCalls).toEqual([[identityKey, ...computeAdditionalData]]);
    expect(compute.invokeCalls).toEqual([[COMPUTED_MARKER, ...invokeAdditionalData]]);
  });
  ...
  it("two computeAndInvoke on the builder throws", () => {
    const { transfers } = testEnv;
    const builder = transfers.alice.build().computeAndInvoke(() => ({
      contractAddress: "0x1",
      computeAdditionalData: [],
      invokeAdditionalData: [],
    }));
