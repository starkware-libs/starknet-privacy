...
///
/// Deposits are filtered by `address` — only deposits made by the querying
...
async fn fetch_aggregated_block_events<E: IEvents>(
    ...
    if !budget.consume(COST_BLOCK_EVENTS_QUERY) {
        return Err(DiscoveryError::InsufficientBudget {
            needed: COST_BLOCK_EVENTS_QUERY,
    ...
    let mut note_to_transaction: HashMap<Felt, Felt> = HashMap::new();
    for event in &block_events {
        let note_id = match &event.content {
            PrivacyPoolEventContent::EncNoteCreated(e) => Some(e.note_id),
            PrivacyPoolEventContent::OpenNoteDeposited(e) => Some(e.note_id),
            _ => None,
        };
        if let Some(note_id) = note_id {
            note_to_transaction.insert(note_id, event.transaction_hash);
        }
    }
    ...
    trace!(
        ...
        num_note_created_events = note_to_transaction.len(),
        num_auxiliary_events = block_events.len(),
        "block_events: pre-merge"
    );

    for note in notes {
        if let Some(&transaction_hash) = note_to_transaction.get(&note.note_id) {
            let tx = transactions
    ...
    for event in block_events {
        if let Some(tx) = transactions.get_mut(&event.transaction_hash) {
            match event.content {
                PrivacyPoolEventContent::Deposit(deposit) if deposit.user_address == address => {
...
/// by `address` at the RPC level (`get_withdrawal_events`). Note that the same
...
async fn fetch_aggregated_withdrawal_events<E: IEvents>(
    ...
    let events = backend
        .get_withdrawal_events(address, from_block, to_block)
        .await?;
