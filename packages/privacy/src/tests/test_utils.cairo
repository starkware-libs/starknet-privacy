...
use privacy::objects::{
    ClientAction, EncChannelInfo, EncNote, EncPrivateKey, EncSubchannelInfo, NewNote, NotePath,
    ServerAction,
};
...
pub(crate) impl UserImpl of UserTrait {
    ...
    fn register(self: @User, random: felt252) -> Span<ServerAction> {
        self
            .compile_client_actions(
                client_actions: [ClientAction::Register((*self.private_key, random))].span(),
            )
    }
    ...
    fn get_enc_private_keys_history(self: @User) -> Span<EncPrivateKey> {
        self.privacy.views.get_enc_private_keys_history(user_addr: *self.address)
    }
    ...
    fn replace_key(self: @User, random: felt252) -> Span<ServerAction> {
        self
            .compile_client_actions(
                client_actions: [ClientAction::ReplaceKey((*self.private_key, random))].span(),
            )
    }
    ...
    /// Returns the random value generated by the user for the key replacement.
    fn replace_key_e2e(ref self: User) -> felt252 {
        let (random, actions) = self.replace_key_with_generated_random();
        self.privacy.server.execute_actions(:actions);
        random
    }
