...
- A **block hash**: `{"block_hash": "0x..."}` — pins reads to a specific confirmed block.
- A **block number**: `{"block_number": N}` — pins reads to a specific block height.
...
- `block_ref`: Optional. Block identifier (`{"block_hash":"0x..."}`, `{"block_number":N}`, or a tag like `"pre_confirmed"`) to pin storage reads to. Can be specified on any request, including the first. When omitted, the server uses the current head.
