...
pub trait IClientSide<T> {
    ...
    /// # Errors
    ///
    /// * `Errors::NOTE_AMOUNT_MUST_BE_NON_ZERO`
    /// * `Errors::NOTE_TOKEN_ZERO_ADDRESS`
    /// * `Errors::NOTE_OWNER_ZERO_ADDRESS`
    /// * `Errors::NOTE_OWNER_MISMATCH`
    /// * `Errors::NOTE_TOKEN_MISMATCH`
    /// * `Errors::NOTE_SUM_MISMATCH`
    ///
    /// # Access Control
    ///
    /// * The caller must be the owner of the input notes.
    ///
    /// # Emits
    ///
    /// N/A
    ...
    /// 2. Return the hashes of the output notes.
    fn transfer(self: @T, input: Span<Note>, output: Span<Note>) -> Span<felt252>;
}

// TODO: Update struct according to actual ID values.
