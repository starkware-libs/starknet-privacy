...
fn test_enc_subchannel_info_is_all_zero() {
    ...
    assert_eq!(enc_subchannel_info.is_all_zero(), false);
    enc_subchannel_info.random = Zero::zero();
    assert_eq!(enc_subchannel_info.is_all_zero(), true);
}

#[test]
fn test_enc_subchannel_info_is_all_non_zero() {
    let mut enc_subchannel_info = EncSubchannelInfo {
        random: 'RANDOM'.try_into().unwrap(), enc_token: 'ENC_TOKEN'.try_into().unwrap(),
    };
    assert_eq!(enc_subchannel_info.is_all_non_zero(), true);
    enc_subchannel_info.random = Zero::zero();
    assert_eq!(enc_subchannel_info.is_all_non_zero(), false);
    enc_subchannel_info.random = 'RANDOM'.try_into().unwrap();
    enc_subchannel_info.enc_token = Zero::zero();
    assert_eq!(enc_subchannel_info.is_all_non_zero(), false);
    enc_subchannel_info.random = Zero::zero();
...
fn test_enc_private_key_is_all_zero() {
    ...
    assert_eq!(enc_private_key.is_all_zero(), false);
    enc_private_key.ephemeral_pubkey = Zero::zero();
    assert_eq!(enc_private_key.is_all_zero(), true);
}
