...
use privacy::objects::{
    DepositToOpenNoteInput, EncChannelInfo, EncOutgoingChannelInfo, EncPrivateKey,
    EncSubchannelInfo, EncUserAddr, Note, TokenBalances, TokenBalancesTrait,
};
...
pub(crate) impl UserImpl of UserTrait {
    ...
    /// Execute _deposit_to_open_note via the echo executor (Invoke path).
    ...
    fn _deposit_to_open_note(
        ...
        let depositor = *self.address;
        self.privacy._deposit_to_open_note(:note_id, :depositor, :token_addr, :amount);
    }

    /// Safe version of `_deposit_to_open_note` that returns a Result.
    /// Assumes the user already has sufficient token balance and approval.
    fn safe_deposit_to_open_note(
        self: @User, note_id: felt252, token_addr: ContractAddress, amount: u128,
    ) -> Result<(), Array<felt252>> {
        let depositor = *self.address;
        self.privacy.safe_deposit_to_open_note(:note_id, :depositor, :token_addr, :amount)
    }
    ...
    fn fund_and_deposit_to_open_note(self: @User, token: Token, note_id: felt252, amount: u128) {
        self.increase_token_balance(:token, :amount);
        self.approve(:token, amount: amount.into());
        self._deposit_to_open_note(:note_id, token_addr: token.contract_address(), :amount);
    }
...
pub(crate) impl PrivacyCfgImpl of PrivacyCfgTrait {
    ...
    fn _deposit_to_open_note(
        ...
        let actions = self
            ._deposit_to_open_note_actions(:note_id, :depositor, :token_addr, :amount);
        self.apply_actions(:actions);
    }
    ...
    fn _deposit_to_open_note_actions(
        ...
        let deposit = DepositToOpenNoteInput { note_id, depositor, token: token_addr, amount };
