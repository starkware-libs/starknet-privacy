 ...
 * `computeData` is forwarded to the target's `privacy_compute` (after the derived identity key);
 * `invokeData` is forwarded to its `privacy_invoke_with_computation` (after the compute result).
 */
export type ComputeAndInvokeDetails = {
  contractAddress: string;
  computeData?: CallDetails["calldata"];
  invokeData?: CallDetails["calldata"];
};
...
export interface PrivateTransfersBuilder {
   ...
   * identity key and `computeData`), then invokes it with the compute result and `invokeData`.
