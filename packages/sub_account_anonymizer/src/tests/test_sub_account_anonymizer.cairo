...
#[test]
fn test_invoke_executes_and_sweeps_open_note() {
    ...
    let deposits = components
        .invoke(
            :commitment,
            invokes: array![pay_out_call(components.mock_dapp, token, AMOUNT)].span(),
            open_notes: array![(NOTE_ID, token)].span(),
        );

    // One deposit returned, matching the swept token/amount.
...
fn test_sub_account_is_reused_per_commitment() {
    ...
    let no_invokes = array![].span();
    let no_notes = array![].span();

    components.invoke(commitment: commitment_a, invokes: no_invokes, open_notes: no_notes);
    ...
    components.invoke(commitment: commitment_b, invokes: no_invokes, open_notes: no_notes);
...
fn test_invoke_only_privacy_contract() {
    ...
    let result = safe
        .privacy_invoke_with_computation(
            commitment: 0, invokes: array![].span(), open_notes: array![].span(),
        );
