...
/// `enc_token = h(ENC_TOKEN_TAG, channel_key, index, 0, random) + token`
pub(crate) fn encrypt_subchannel_info(
    channel_key: felt252, index: usize, token: ContractAddress, random: felt252,
) -> EncSubchannelInfo {
    let enc_token = compute_enc_token_hash(:channel_key, :index, :random) + token.into();
...
/// Assumes all the inputs (except `index`) are not zero, and `random` is 120 bits.
///
/// `enc_amount = packing(random, (h(ENC_AMOUNT_TAG, channel_key, token, index, 0, random) + amount)
/// %
/// 2^128)`.
pub(crate) fn encrypt_note_amount(
    channel_key: felt252, token: ContractAddress, index: usize, random: u128, amount: u128,
) -> felt252 {
    let enc_amount = (compute_enc_amount_hash(:channel_key, :token, :index, :random)
        + amount.into())
        .into() % TWO_POW_128;
...
pub(crate) fn decrypt_note_amount(
    ...
    let pad: u256 = compute_enc_amount_hash(:channel_key, :token, :index, :random)
        .into() % TWO_POW_128;
