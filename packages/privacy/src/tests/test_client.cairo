...
fn test_create_note_self_note() {
    ...
    let create_note_input = user
        .new_open_note_with_generated_random(
            recipient: user, token: token_address, index: note_index,
        );
...
fn test_create_note_twice() {
    ...
    let note_3 = user_1
        .new_open_note_with_generated_random(recipient: user_2, token: token_address, index: 2);
    ...
    let note_4 = user_1
        .new_open_note_with_generated_random(recipient: user_2, token: token_address, index: 3);
...
fn test_create_open_note_assertions() {
    ...
    user.set_viewing_key_e2e();
    let create_note_input = user
        .new_open_note_with_generated_random(recipient: user, token: token_address, index: 0);
...
fn test_create_note_subchannel_not_found_wrong_token() {
    ...
    // Open note.
    let create_note_input = user_1
        .new_open_note_with_generated_random(recipient: user_2, token: wrong_token, index: 0);
    let result = user_1.safe_create_open_note(:create_note_input);
    assert_panic_with_felt_error(:result, expected_error: errors::SUBCHANNEL_NOT_FOUND);
}
...
fn test_create_open_note_stores_token() {
    ...
    let create_note_input = user_1
        .new_open_note_with_generated_random(
            recipient: user_2, token: token_address, index: note_index,
        );
...
#[test]
fn test_create_open_note_decrypt_user_addr() {
    let mut test = Default::default();
    let mut user_1 = test.new_user();
    let mut user_2 = test.new_user();
    // Setup.
    ...
    // Compliance should be able to decrypt the user address from the OpenNoteCreated event.
    let events = spy_events.get_events().emitted_by(contract_address: test.privacy.address).events;
    assert_eq!(events.len(), 1);
    let (_, event) = events[0];
    // For OpenNoteCreated, enc_user_addr is a #[key] field, so it's in event.keys.
    // keys[0] is the event selector, followed by enc_user_addr fields.
    let enc_user_addr = EncUserAddr {
        compliance_public_key: *event.keys[1],
        ephemeral_pubkey: *event.keys[2],
        enc_user_addr: *event.keys[3],
    };
    let decrypted_user_addr = test.compliance.decrypt_user_addr(:enc_user_addr);
    assert_eq!(decrypted_user_addr, user_1.address);
}
...
fn test_actions_out_of_order() {
    ...
    let open_note = user
        .new_open_note_with_generated_random(recipient: user, token: token_address, index: 1);
    let client_actions = [
...
fn test_client_execute_writes() {
    ...
    let create_open_note_input = user
        .new_open_note_with_generated_random(recipient: user, token: token_address, index: index);
