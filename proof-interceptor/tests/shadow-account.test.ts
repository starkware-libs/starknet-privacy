...
/**
 * The `invoke_additional_data` the SDK's shadow account builder produces: the
 * anonymizer's `privacy_invoke_with_computation` calldata without its leading
 * `identity_commitment`, which the pool prepends from the compute result.
 */
function invokeAdditionalData(openNotes: unknown[]): string[] {
