#!/usr/bin/env python3
"""Emits the committed cross-language screening vectors.

The reference TS signer (scripts/address_validation_signer/ts/validation_signer.ts)
is the single producer of the signatures: this script shells out to its CLI for
each canonical input and assembles fixtures/screening-vectors.json. The privacy
contract's Cairo verifier tests consume the fixture (rendered to Cairo by
scripts/gen_cairo_screening_vectors.py); it is also the intended vector source
for the off-chain screening signer. Signing is deterministic (RFC6979), so this
is idempotent — CI fails if the committed fixture drifts.
...
def main() -> None:
    ...
    # newline="\n" keeps the committed fixture LF-only on every platform.
