...
describe("SubAccount anonymizer compute-and-invoke on devnet", () => {
  ...
  it("dapp payout collected via the sub-account settles into an open note", async () => {
    ...
    // `compute_data` feeds privacy_compute(identity_key, dapp_name, seq_nonce); the pool prepends
    ...
    const { callAndProof } = await transfers.alice
      ...
      .computeAndInvoke((args) => {
        ...
        if (!openNote) {
          throw new Error("Expected one open note for compute-and-invoke");
        }
        // `invokeData` carries `privacy_invoke_with_computation`'s args after the
        ...
        const invokeData = new CallData(subAccount.anonymizerAbi)
    ...
    const filledAmount = usdNotes.reduce((sum, note) => sum + note.amount, 0n);
    expect(filledAmount).toBe(payoutAmount);

    // Funds ended in the privacy pool; the dapp and anonymizer hold nothing.
    expect(await balanceOf(de.privacy.address)).toBeGreaterThanOrEqual(
      payoutAmount,
    );
