...
fn test_execute_assertions() {
    ...
    let result = user_invalid.safe_execute(client_actions: [].span());
    assert_panic_with_felt_error(:result, expected_error: errors::INVALID_SIGNATURE);

    // TODO: Catch server errors.

    // Catch INVALID_CALLER.
    let result = user.safe_execute_without_cheat_caller(client_actions: [].span());
    ...
    let result = user.safe_execute(client_actions: [ClientAction::UseNote(note_1_path),].span());
    assert_panic_with_felt_error(:result, expected_error: errors::FINAL_BALANCE_MUST_BE_ZERO);
    ...
    let result = user.safe_execute(client_actions: [ClientAction::CreateNote(note_2),].span());
    assert_panic_with_felt_error(:result, expected_error: errors::NEGATIVE_INTERMEDIATE_BALANCE);
...
fn test_execute_writes() {
    ...
    let result = user.safe_execute(client_actions: client_actions);
