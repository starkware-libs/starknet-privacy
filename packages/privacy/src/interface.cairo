...
pub trait IServer<T> {
    ...
    /// - For [`TransferTo`](privacy::actions::ServerAction::TransferTo) actions, the contract must
    ...
    /// ERC20 contract).
    ///
    /// **Errors for [`TransferTo`](privacy::actions::ServerAction::TransferTo) action:**
    /// - `INSUFFICIENT_BALANCE`: Thrown if the contract has insufficient token balance (from ERC20
    /// contract).
    ///
    /// #### Access Control
