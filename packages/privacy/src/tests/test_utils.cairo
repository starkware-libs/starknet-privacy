...
fn test_encrypt_decrypt_note_amount() {
    ...
    for amount in amounts.span() {
        ...
        nonce += 1;
        let random_120_bits: felt252 = (hash(['RANDOM', nonce.into()].span()).into() % TWO_POW_120)
            .try_into()
            .unwrap();
...
fn test_packing_unpacking() {
    ...
    for (value_1, value_2) in values.span() {
        let packed_value = packing(value_1: *value_1, value_2: *value_2);
...
fn test_packing_unpacking_random() {
    ...
    for _ in 0..100_u32 {
        nonce += 1;
        let value_1_120_bits: felt252 = (hash(['VALUE_1', nonce.into()].span())
            .into() % TWO_POW_120)
            .try_into()
            .unwrap();
        nonce += 1;
