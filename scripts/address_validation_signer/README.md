...
across both. The signature `(r, s)` will differ between runs and between
languages because STARK ECDSA uses a non-deterministic nonce; both are valid
against the shared hash.
...
checks during contract development.
