 ...
 *     call_set_hash,      // poseidon(CALL_SET_TYPE_HASH, poseidon([hash(call) for call in calls]))
 *   ])
...
const CALL_SET_TYPE_HASH = 0x3394eb24ecf5791f5463348da0e37ffc950765ca43e296497545bfb76cc8d7en;
...
/** SNIP-12 `CallSet { calls }` struct hash, matching `privacy::snip12::CallSet`. */
function hashCallSet(calls: Call[]): bigint {
  return poseidon([CALL_SET_TYPE_HASH, poseidon(calls.map(hashCall))]);
}
 ...
 * must equal `privacy::snip12::compute_call_set_hash(accountAddress, calls)` under the same chain id.
 */
export function computeCallSetHash(
  ...
  return poseidon([STARKNET_MESSAGE, domainHash, toFelt(accountAddress), hashCallSet(calls)]);
}
