...
// Cryptographically verify a returned signature instead of recomputing it
// with signScreening, which would reproduce its own bugs on both sides of the
// comparison. computeScreeningMessageHash is itself pinned to the committed
// vectors in signing.test.ts.
...
describe("POST /screen signing", () => {
  ...
  // Wired like production: the mock upstream is dispatched by elliptic.url.
  // mockForward stands in for the live forwarder so each test can inject the
  // exact Elliptic HTTP response (clean / 404 / scored-block) its branch
  // needs — responses the always-404 mock upstream can't produce on demand.
  ...
  it("signs an Elliptic-backed allow over the configured chain id", async () => {
    ...
    // The reported issued_at must be the server's own current second. Bound it
    // before recomputing, so a server that signs over a different value than it
    // reports (ms instead of seconds, or 0) cannot pass the self-consistent
    // recomputation below.
    ...
    // The signature must verify against the fixture's signer public key over
    // the digest of the server-chosen timestamp, the configured chain id, and
