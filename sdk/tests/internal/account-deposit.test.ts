/**
 * Tests for `buildAccountDepositInvoke` — SNIP-9 outside execution dispatched via `CallAnonymizer`.
 */
...
describe("buildAccountDepositInvoke (via builder)", () => {
  ...
  it("returns a single apply_actions call (no deploy)", async () => {
    ...
    const result = await depositViaBuilder(
      transfers.alice,
      {
        callAnonymizerAddress: toHex(CALL_ANONYMIZER_ADDRESS),
  ...
  it("inner signed call is [token.transfer(anonymizer, amount)]", async () => {
    const { env, transfers } = testEnv;
    await bootstrapAlice();
    const { signer } = makeEphemeralSigner();
    const accountAddress = `0x${"a".repeat(63)}1`;
    ...
    expect(toBigInt(calls[0].Selector)).toBe(toBigInt(hash.getSelectorFromName("transfer")));
  });
