...
fn test_store_and_apply_stored_actions_happy_path() {
    ...
    // Apply later — no proof or fee needed.
    test.privacy.apply_stored_actions(:actions_hash);
    assert!(test.privacy.channel_exists(:channel_marker));

    // Cannot re-apply.
    let result = test.privacy.safe_apply_stored_actions(:actions_hash);
    assert_panic_with_felt_error(
...
fn test_apply_stored_actions_rejects_missing_entry() {
    ...
    let result = test.privacy.safe_apply_stored_actions(actions_hash: unknown_hash);
    assert_panic_with_felt_error(:result, expected_error: errors::NO_STORED_ACTIONS);
}

#[test]
fn test_store_actions_requires_valid_proof() {
    // The proof must be validated at store time, so apply needs no proof.
    let mut test: Test = Default::default();
    let (_, channel_marker) = test.mock_new_channel();
    let storage_address = map_entry_address(
        map_selector: selector!("channel_exists"), keys: [channel_marker].span(),
    ...
    // No proof facts cheated → store must revert with EMPTY_PROOF_FACTS.
    let result = test.privacy.safe_store_actions_without_cheat(:actions);
    assert_panic_with_felt_error(:result, expected_error: errors::EMPTY_PROOF_FACTS);

    // After the failed attempt, the entry is still empty and a valid proof allows store.
    let actions_hash = test.privacy.store_actions(:actions);
    test.privacy.apply_stored_actions(:actions_hash);
    assert!(test.privacy.channel_exists(:channel_marker));
}
