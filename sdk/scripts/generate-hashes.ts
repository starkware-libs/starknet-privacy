...
// Split source into chunks by function definition
// Pattern: doc comments followed by pub(crate) fn
const functionChunks = cairoSource.split(/(?=(?:\/\/\/[^\n]*\n)+\s*pub\(crate\)\s+fn\s+)/);

for (const chunk of functionChunks) {
  // Match the function signature
  const sigMatch = chunk.match(
    /((?:\/\/\/[^\n]*\n)+)\s*pub\(crate\)\s+fn\s+(\w+)\s*\(\s*([\s\S]*?)\s*\)\s*->\s*felt252\s*\{([\s\S]*?)\n\}/
  );

  if (!sigMatch) continue;

  const [, docComment, name, paramsStr, bodyStr] = sigMatch;
...
function convertDocComment(docComment: string): string {
  return docComment
    .split("\n")
    .map((line) => {
...
for (const func of functions) {
  // Doc comment
  tsLines.push("/**");
  tsLines.push(convertDocComment(func.docComment));
  tsLines.push(" */");
