...
fn test_compute_message_hash_depends_on_class_hash() {
    ...
    payload.serialize(ref l1_data_valid);
    real_class_hash.serialize(ref l1_data_valid);
    ...
    payload.serialize(ref l1_data_invalid);
    let zero_class_hash: ClassHash = Zero::zero();
    zero_class_hash.serialize(ref l1_data_invalid);
