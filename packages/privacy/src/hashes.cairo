...
pub mod domain_separation {
    ...
    /// Tags for the `EncOutgoingChannelInfo` struct.
    // TODO: Now using "outgoing" instead of "enc_outgoing_channel_info" to fit in a single felt.
    pub mod enc_outgoing_channel_info {
        pub const ENC_RECIPIENT_ADDR_TAG: felt252 = 'outgoing:enc_recipient_addr:v1';
    }
    /// Tag for `outgoing_channel_key`.
    pub const OUTGOING_CHANNEL_KEY_TAG: felt252 = 'outgoing_channel_key:v1';
}
...
///
/// Returns `h(ENC_TOKEN_TAG, channel_key, salt)`
pub(crate) fn compute_enc_token_hash(channel_key: felt252, salt: felt252) -> felt252 {
    hash([enc_subchannel_info::ENC_TOKEN_TAG, channel_key, salt].span())
}
...
pub(crate) fn compute_enc_recipient_addr_hash(
    ...
    hash(
        [
            enc_outgoing_channel_info::ENC_RECIPIENT_ADDR_TAG, sender_addr.into(),
            sender_private_key, index.into(), salt,
        ]
            .span(),
    )
}
...
///
/// Returns `h(ENC_AMOUNT_TAG, channel_key, salt)`.
pub(crate) fn compute_enc_amount_hash(channel_key: felt252, salt: u128) -> felt252 {
    hash([ENC_AMOUNT_TAG, channel_key, salt.into()].span())
}
