...
/// Input for the `CreateEncryptedNote` action.
///
/// Includes a salt to guarantee one-time key usage, preventing privacy-related data leakage
/// if a transaction is reverted and the same note id is reused.
#[derive(Serde, Copy, Drop, PartialEq, Debug)]
pub struct CreateEncryptedNoteInput {
    ...
    /// Must be 120 bits and >= CREATE_NOTE_MIN_SALT (= 2).
    /// (`salt = 1` is reserved for the non-encrypted note, and `salt = 0` indicates the note does
    /// not exist.)
    pub salt: u128,
}
...
pub enum ClientAction {
    ...
    CreateEncryptedNote: CreateEncryptedNoteInput,
    /// Creates a new open note (deposit slot).
...
pub(crate) impl ClientActionImpl of ClientActionTrait {
    ...
    fn intended_phase(self: @ClientAction) -> u8 {
        match self {
            ...
            ClientAction::CreateEncryptedNote(_) => Self::CREATE_NOTES_PHASE,
