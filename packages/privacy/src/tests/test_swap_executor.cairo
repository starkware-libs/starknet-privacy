...
fn test_swap_basic(preexisting_balance: u128) {
    ...
    // Execute swap (caller must be the privacy contract).
    cheat_caller_address_once(
        contract_address: test.swap_executor.address, caller_address: test.privacy.address,
    );
    let received = test
        .swap_executor
        .swap(
...
#[test]
fn test_swap_caller_missing_deposit_to_open_note() {
