...
fn test_use_deposited_open_note() {
    ...
    assert!(stored_note.packed_value.is_non_zero());
    ...
    // Verify the actions contain nullifier write and note creation.
...
fn test_client_execute_writes() {
    ...
    let create_open_note_input = CreateOpenNoteInput {
        recipient_addr, recipient_public_key, token: token_address, index: index + 1, depositor,
    };
