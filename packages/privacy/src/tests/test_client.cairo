...
use privacy::tests::test_utils::{
    AssertEncNoteCompilesToActionsTrait, PrivacyCfgTrait, Test, TestTrait, UserTrait,
    decrypt_channel_info, decrypt_subchannel_token,
};
use privacy::utils::{
...
fn test_compile_client_actions() {
    ...
    let random = user_1.open_channel_e2e(recipient: user_2, token: token.contract_address());
    let actions = user_1
        .compile_client_actions(
            client_actions: [
                ClientAction::OpenSubchannel(
                    (user_2.address, expected_channel_key, 0, token.contract_address(), random),
                ),
            ]
                .span(),
        );
    let expected_subchannel_id = user_1
        .compute_subchannel_id(recipient: user_2, token: token.contract_address());
    let expected_subchannel_key = user_1
        .compute_subchannel_key(recipient: user_2, token: token.contract_address(), index: 0);
    let expected_enc_subchannel_info = user_1
        .compute_enc_subchannel_info(
            recipient: user_2, token: token.contract_address(), random: random,
        );
    let subchannel_exists_storage_path_felt = map_entry_address(
        map_selector: selector!("subchannel_exists"), keys: [expected_subchannel_id].span(),
    );
    ...
    let amount = 1;
    let index = 0;
    let note = user_1.new_note(recipient: user_2, token: token.contract_address(), :amount, :index);
    let actions = user_1.create_note(:note);
    let enc_note = user_1
        .compute_enc_note(recipient: user_2, token: token.contract_address(), :index, :amount);
    let storage_path_felt = map_entry_address(
        map_selector: selector!("notes"), keys: [enc_note.id].span(),
    );
    let expected_actions = array![
        ServerAction::WriteIfZero((storage_path_felt, enc_note.enc_amount)),
    ]
        .span();
    assert_eq!(actions, expected_actions);
}
