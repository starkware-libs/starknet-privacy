...
pub mod Privacy {
    ...
    use privacy::snip12::{DepositorValidation, ScreeningAttestation, verify_depositor_validation};
                ...
                match *action {
                    ...
                    ServerAction::TransferFrom(input) => {
                        if deposit_depositor.is_zero() {
                            ...
                            assert(
                                deposit_depositor == input.from_addr, errors::MULTIPLE_DEPOSITORS,
                            );
                        }
            ...
            match screening {
                ...
                Option::Some(attestation) => {
                    ...
                    let validation = DepositorValidation {
                        depositor, issued_at: attestation.issued_at,
                    };
                    assert(
                        verify_depositor_validation(
                            validation, attestation.signature, self.screener_public_key.read(),
                        ),
