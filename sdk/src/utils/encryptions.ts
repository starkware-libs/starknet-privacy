...
export const encryptions = {
   ...
   */
  decryptChannelInfo: (encrypted: EncChannelInfo, recipientPrivateKey: bigint): ChannelInfo => {
    ...
    const key =
      (((encrypted.encChannelKey - compute_enc_channel_key_hash(sharedX)) % FIELD_PRIME) +
        FIELD_PRIME) %
      FIELD_PRIME;
    const sender =
      (((encrypted.encSenderAddr - compute_enc_sender_addr_hash(sharedX)) % FIELD_PRIME) +
        FIELD_PRIME) %
      FIELD_PRIME;
  ...
  decryptSubchannelInfo: (
    ...
    const { salt, enc_token } = encrypted;
    const encTokenHash = compute_enc_token_hash(channelKey, index, salt);
