...
import { CairoCustomEnum, CallData, type BigNumberish } from "starknet";
...
const DAPP_CALLS = [{ to: TOKEN, selector: "0x1", calldata: ["0x2"] }];
const OPEN_NOTES = [
  {
    note_id: "0x3",
    token: TOKEN,
...
function computeAndInvokeAction(
  target = ANONYMIZER_ADDR,
  openNotes: unknown[] = OPEN_NOTES
): CairoCustomEnum {
  const invokeData: BigNumberish[] = anonymizerCallData
  ...
  return new CairoCustomEnum({
    ComputeAndInvoke: {
      ...
      invoke_additional_data: invokeData,
    },
  });
}
...
/** Silences the refusal log a delegated depositor this cannot name writes, and returns its spy. */
function silenceErrorLog(): ReturnType<
...
describe("getScreenedAddress", () => {
  ...
  it("screens the shadow account of a Delegated interaction, not the anonymizer", async () => {
    ...
    expect(screened.kind).toBe("one");
    expect(screened).not.toEqual({ kind: "one", address: ANONYMIZER_ADDR });
  });
  ...
  it("reads no policy for an interaction that settles no open note", async () => {
    // The pool consults the policy list only when an invoke returns deposits, and an interaction
    // settling no note returns none.
    const { screened, asked } = await subjectOf([
  ...
  it("refuses a delegated compute-invoke whose address it cannot compute", async () => {
    ...
    expect(screened).toEqual({ kind: "unimplementedDelegate" });
  ...
  it("says what is unimplemented when it refuses, without naming an address", async () => {
    const errorSpy = silenceErrorLog();
    ...
    expect(logged).toContain("delegated_screening_unimplemented");
