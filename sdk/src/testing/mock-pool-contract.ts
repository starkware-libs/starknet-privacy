...
export class MockPoolContract implements MockContract, PoolContractInterface {
  ...
  // Screening capability this mock pool advertises. 0 = current pool (no
  // `screening_version` semantics, no attestation); >= 1 = screening-capable.
  private screeningVersionValue = 0n;
  ...
  screening_version(): bigint {
    return this.screeningVersionValue;
  }

  /** Simulate the new (>=1) or current (0) pool for screening-capability detection. */
  setScreeningVersion(version: bigint): void {
    this.screeningVersionValue = version;
  }
