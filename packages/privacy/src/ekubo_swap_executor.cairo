//! Ekubo swap executor: executes a single-hop swap on an Ekubo Router and deposits
//! the output to an open note on the caller (privacy contract).
//!
...
pub mod EkuboSwapExecutor {
    ...
    pub impl EkuboSwapExecutorImpl of IEkuboSwapExecutor<ContractState> {
        fn privacy_invoke(
            ...
            // Validate in_token and out_token are the pool's token0 and token1.
            let (t0, t1) = (pool_key.token0, pool_key.token1);
            assert(
                (in_token == t0 && out_token == t1) || (in_token == t1 && out_token == t0),
            ...
            // Router expects to pull input from caller (our mock does transfer_from).
            // For a real Ekubo Router that holds tokens, the caller would transfer to Router first.
            in_erc20.approve(spender: router_addr, amount: in_amount.into());
