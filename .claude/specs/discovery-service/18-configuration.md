...
```toml
[privacy_pool]
contract_address = "0x..."  # REQUIRED — no default
...
|---|---|---|
| `CONTRACT_ADDRESS` | `privacy_pool.contract_address` | — (required) |
...
Only `privacy_pool.contract_address` is required. Must be set via config file or `CONTRACT_ADDRESS` env var before validation passes.

## Loading Sequence

1. Parse `--config` CLI arg
2. If config path provided: read file → expand env vars → parse TOML
3. If no config path: use `ServiceConfig::default()` (all fields `None`)
4. Apply env var overrides (`apply_env_overrides`)
5. Validate required fields (`validate`)
6. Construct component configs from resolved values
