...
theorem incoming_notes
    ...
    (
      ...
      |>.map (λ tx ↦ tx.sum_use_note_amounts token)
      |>.sum
    ) := by
  rw [←sum_of_used_notes_to_scanned_notes bob token]
  rw [←filtered_note_actions_to_tx_actions]
  unfold sum_use_note_amounts used_note_actions ActionFunc.use ActionFuncRes.from_use
  simp only [List.filter_filterMap, List.map_filterMap]
  apply congrArg
  apply List.filterMap_congr
  intro action h_action
  cases h_some: filter_UseNote action
  case none => simp
