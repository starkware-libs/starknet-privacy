...
| `cacheTtlSeconds` | How long to cache the config before re-reading from Secret Manager |
...
Update the secret value in Secret Manager. The proxy will pick up the new config after `cacheTtlSeconds` expires (no redeploy needed).
