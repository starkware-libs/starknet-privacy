...
export async function getScreenedAddress(
  ...
  if (openNoteDepositor !== null) {
    switch (await policies.getPolicy(openNoteDepositor.address)) {
      case "Exempt":
        break;
...
function getOpenNoteDepositor(
  ...
  for (const action of actions) {
    const invokeAction = asInvokeAction(action);
    if (invokeAction === null) continue;
...
function asInvokeAction(action: CairoCustomEnum): InvokeAction | null {
  const variant = action.activeVariant();
  return variant === "InvokeExternal" || variant === "ComputeAndInvoke"
    ? variant
    : null;
}
