...
fn test_set_viewing_key() {
    ...
    let expected_actions = [
        ServerAction::Write((public_key_storage_path_felt, public_key)),
        ServerAction::WritePrivateKey((enc_private_key_storage_path_felt, enc_private_key)),
    ]
        .span();
...
fn test_transfer() {
    ...
    let note_index = 0;
    let note = user_1
        .new_note_with_new_nonce(recipient: user_1, :token, :amount, index: note_index);
    ...
    let note = user_1
        .new_note_with_new_nonce(recipient: user_2, :token, :amount, index: note_index);
    let actions = user_1
    ...
    let enc_note = user_1
        .compute_enc_note(recipient: user_2, :token, index: note_index, :amount, nonce: note.nonce);
...
fn test_transfer_to_self() {
    ...
    let enc_note = user_1
        .compute_enc_note(recipient: user_1, :token, index: note_index, :amount, nonce: note.nonce);
    let storage_path_felt_nullifier = map_entry_address(
        map_selector: selector!("nullifiers"), keys: [expected_nullifier].span(),
    );
    let storage_path_felt_note = map_entry_address(
...
fn test_transfer_one_to_many() {
    ...
    let note_1 = user_1
        .new_note_with_new_nonce(recipient: user_2, :token, amount: amount_1, index: note_index);
    let note_2 = user_1
        .new_note_with_new_nonce(recipient: user_3, :token, amount: amount_2, index: note_index);
    ...
    let enc_note_1 = user_1
        .compute_enc_note(
            recipient: user_2, :token, index: note_index, amount: amount_1, nonce: note_1.nonce,
        );
    let enc_note_2 = user_1
        .compute_enc_note(
            recipient: user_3, :token, index: note_index, amount: amount_2, nonce: note_2.nonce,
        );
    let storage_path_felt_nullifier = map_entry_address(
        map_selector: selector!("nullifiers"), keys: [expected_nullifier].span(),
    );
...
fn test_transfer_many_to_many() {
    ...
    let note_index = 0;
    let note = user_1
        .new_note_with_new_nonce(recipient: user_3, :token, :amount, index: note_index);
    user_1.cheat_create_note_e2e(:note);
    ...
    let enc_note_1 = user_3
        .compute_enc_note(
            recipient: user_1, :token, index: note_index, :amount, nonce: note_1.nonce,
        );
    let enc_note_2 = user_3
        .compute_enc_note(
            recipient: user_2, :token, index: note_index, :amount, nonce: note_2.nonce,
        );
...
fn test_transfer_assertions() {
    ...
    let note = user_1.new_note_with_new_nonce(recipient: user_1, :token, amount: 1, index: 0);
    user_1.cheat_create_note_e2e(:note);
    ...
    let result = user_1
        .safe_transfer(
            notes_to_use: [use_note_input].span(),
            notes_to_create: [
                CreateNoteInput { nonce: TWO_POW_120.try_into().unwrap(), ..create_note_input }
            ]
                .span(),
        );
    assert_panic_with_felt_error(:result, expected_error: errors::NONCE_EXCEEDS_120_BITS);
...
fn test_open_subchannel() {
    ...
    let (nonce, channel_output) = user_1
        .internal_open_subchannel_with_new_nonce(recipient: user_2, :token, index: 0);
    ...
    let expected_enc_subchannel_info = user_1
        .compute_enc_subchannel_info(recipient: user_2, :token, :nonce);
...
fn test_open_subchannel_self_channel() {
    ...
    let (nonce, channel_output) = user
        .internal_open_subchannel_with_new_nonce(recipient: user, :token, index: 0);
...
fn test_open_subchannel_assertions() {
    ...
    let mut user_2 = test.new_user();
    let token = test.mock_new_token();
    let nonce = user_1.get_nonce().into();
    let index = 0;

    // Catch ZERO_USER_ADDR.
    let mut user_zero_addr = user_1;
    user_zero_addr.address = Zero::zero();
    let result = user_zero_addr.safe_open_subchannel(recipient: user_2, :token, :index, :nonce);
    assert_panic_with_felt_error(:result, expected_error: errors::ZERO_USER_ADDR);

    // Catch ZERO_RECIPIENT_ADDR.
    let mut user_zero_addr = user_2;
    user_zero_addr.address = Zero::zero();
    let result = user_1.safe_open_subchannel(recipient: user_zero_addr, :token, :index, :nonce);
    assert_panic_with_felt_error(:result, expected_error: errors::ZERO_RECIPIENT_ADDR);

    // Catch ZERO_CHANNEL_KEY.
    let result = user_1
        .safe_open_subchannel_with_channel_key(
            recipient: user_2, :token, :index, :nonce, channel_key: Zero::zero(),
        );
    assert_panic_with_felt_error(:result, expected_error: errors::ZERO_CHANNEL_KEY);
    ...
    user_zero_public_key.public_key = Zero::zero();
    let result = user_1
        .safe_open_subchannel(recipient: user_zero_public_key, :token, :index, :nonce);
    assert_panic_with_felt_error(:result, expected_error: errors::ZERO_RECIPIENT_PUBLIC_KEY);

    user_2.set_viewing_key_e2e();

    // Catch INVALID_CHANNEL - sender is not registered.
    let result = user_1.safe_open_subchannel(recipient: user_2, :token, :index, :nonce);
    ...
    let mut user_1_wrong_addr = user_1;
    user_1_wrong_addr.address = user_2.address;
    let result = user_1_wrong_addr.safe_open_subchannel(recipient: user_2, :token, :index, :nonce);
    assert_panic_with_felt_error(:result, expected_error: errors::INVALID_CHANNEL);

    // Catch INVALID_CHANNEL - wrong recipient_addr.
    let mut user_2_wrong_addr = user_2;
    user_2_wrong_addr.address = user_1.address;
    let result = user_1.safe_open_subchannel(recipient: user_2_wrong_addr, :token, :index, :nonce);
    assert_panic_with_felt_error(:result, expected_error: errors::INVALID_CHANNEL);
    ...
    let result = user_1
        .safe_open_subchannel(recipient: user_2_wrong_public_key, :token, :index, :nonce);
    assert_panic_with_felt_error(:result, expected_error: errors::INVALID_CHANNEL);

    // Catch INVALID_CHANNEL - wrong channel key.
    let result = user_1
        .safe_open_subchannel_with_channel_key(
            recipient: user_2, :token, :index, :nonce, channel_key: channel_key + 1,
        );
    assert_panic_with_felt_error(:result, expected_error: errors::INVALID_CHANNEL);

    // Catch INDEX_NOT_SEQUENTIAL.
    let result = user_1.safe_open_subchannel(recipient: user_2, :token, index: index + 1, :nonce);
    assert_panic_with_felt_error(:result, expected_error: errors::INDEX_NOT_SEQUENTIAL);
...
fn test_open_subchannel_multiple() {
    ...
    let (nonce_1, c1_output) = user_1
        .internal_open_subchannel_with_new_nonce(recipient: user_2, token: token_1, index: 0);
    test.privacy.execute_actions(actions: c1_output);
    let (nonce_2, c2_output) = user_1
        .internal_open_subchannel_with_new_nonce(recipient: user_2, token: token_2, index: 1);
    ...
    let expected_enc_subchannel_info_1 = user_1
        .compute_enc_subchannel_info(recipient: user_2, token: token_1, nonce: nonce_1);
    let expected_enc_subchannel_info_2 = user_1
        .compute_enc_subchannel_info(recipient: user_2, token: token_2, nonce: nonce_2);
    ...
    assert_ne!(expected_enc_subchannel_info_1.nonce, expected_enc_subchannel_info_2.nonce);
    ...
    test.privacy.execute_actions(actions: c1_output);
    let (nonce_2, c2_output) = user_1
        .internal_open_subchannel_with_new_nonce(recipient: user_2, :token, index: 1);
    let expected_subchannel_key_1 = user_1.compute_subchannel_key(recipient: user_2, index: 0);
    let expected_subchannel_key_2 = user_1.compute_subchannel_key(recipient: user_2, index: 1);
    let expected_enc_subchannel_info_1 = user_1
        .compute_enc_subchannel_info(recipient: user_2, :token, nonce: nonce_1);
    let expected_enc_subchannel_info_2 = user_1
        .compute_enc_subchannel_info(recipient: user_2, :token, nonce: nonce_2);
    ...
    test.privacy.execute_actions(actions: c1_output);
    let (nonce_2, c2_output) = user_1
        .internal_open_subchannel_with_new_nonce(recipient: user_2, token: token_2, index: 0);
...
fn test_open_subchannel_multiple_self_channel() {
    ...
    // Multiple subchannels with different tokens.
    let (nonce_1, c1_output) = user
        .internal_open_subchannel_with_new_nonce(recipient: user, token: token_1, index: 0);
    test.privacy.execute_actions(actions: c1_output);
    let (nonce_2, c2_output) = user
        .internal_open_subchannel_with_new_nonce(recipient: user, token: token_2, index: 1);
    ...
    let expected_enc_subchannel_info_1 = user
        .compute_enc_subchannel_info(recipient: user, token: token_1, nonce: nonce_1);
    let expected_enc_subchannel_info_2 = user
        .compute_enc_subchannel_info(recipient: user, token: token_2, nonce: nonce_2);
...
fn test_create_note_self_note() {
    ...
    let amount = 1;
    let note_index = 0;
    let note = user.new_note_with_new_nonce(recipient: user, :token, :amount, index: note_index);
    let actions = user.internal_create_note(:note);
    let expected_enc_note = user
        .compute_enc_note(recipient: user, :token, index: note_index, :amount, nonce: note.nonce);
...
fn test_create_note_twice() {
    ...
    let note_index_1 = 0;
    let note_1 = user_1
        .new_note_with_new_nonce(recipient: user_2, :token, amount: amount_1, index: note_index_1);
    ...
    let note_2 = user_1
        .new_note_with_new_nonce(recipient: user_2, :token, amount: amount_2, index: note_index_2);
    let create_note_2_actions = user_1.internal_create_note(note: note_2);
    let expected_note_1 = user_1
        .compute_enc_note(
            recipient: user_2, :token, index: note_index_1, amount: amount_1, nonce: note_1.nonce,
        );
    let expected_note_2 = user_1
        .compute_enc_note(
            recipient: user_2, :token, index: note_index_2, amount: amount_2, nonce: note_2.nonce,
        );
...
fn test_create_note_twice_same_amount() {
    ...
    let note_index_1 = 0;
    let note_1 = user_1
        .new_note_with_new_nonce(recipient: user_2, :token, :amount, index: note_index_1);
    let create_note_1_actions = user_1.internal_create_note(note: note_1);
    let note_index_2 = note_index_1 + 1;
    test.privacy.execute_actions(actions: create_note_1_actions);
    let note_2 = user_1
        .new_note_with_new_nonce(recipient: user_2, :token, :amount, index: note_index_2);
    let create_note_2_actions = user_1.internal_create_note(note: note_2);
    let expected_enc_note_1 = user_1
        .compute_enc_note(
            recipient: user_2, :token, index: note_index_1, :amount, nonce: note_1.nonce,
        );
    let expected_enc_note_2 = user_1
        .compute_enc_note(
            recipient: user_2, :token, index: note_index_2, :amount, nonce: note_2.nonce,
        );
...
fn test_create_note_zero_recipient_addr() {
    ...
    let note = user_1.new_note_with_new_nonce(recipient: user_2, :token, amount: 1, index: 0);
...
fn test_create_note_zero_token() {
    ...
    let user_2 = test.new_user();
    let note = user_1
        .new_note_with_new_nonce(recipient: user_2, token: Zero::zero(), amount: 1, index: 0);
    user_1.create_note(:note);
}
...
#[test]
#[should_panic(expected: 'NONCE_EXCEEDS_120_BITS')]
fn test_create_note_nonce_exceeds_120_bits() {
    let mut test: Test = Default::default();
    ...
    let note = user_1
        .new_note(
            recipient: user_2, :token, amount: 1, index: 0, nonce: TWO_POW_120.try_into().unwrap(),
        );
    user_1.create_note(:note);
}
...
fn test_create_note_zero_recipient_public_key() {
    ...
    let user_2 = test.new_user();
    let token = test.mock_new_token();
    let mut note = user_1.new_note_with_new_nonce(recipient: user_2, :token, amount: 1, index: 0);
...
fn test_create_note_index_not_sequential() {
    ...
    user_1.open_channel_with_token_e2e(recipient: user_2, :token, subchannel_index: 0);
    let amount = 1;
    let note = user_1.new_note_with_new_nonce(recipient: user_2, :token, :amount, index: 1);
    user_1.create_note(:note);
}
...
fn test_use_note_multiple_notes() {
    ...
    let amount_2 = 2;
    let note_1 = user_1
        .new_note_with_new_nonce(recipient: user_2, :token, amount: amount_1, index: 0);
    let note_2 = user_1
        .new_note_with_new_nonce(recipient: user_2, :token, amount: amount_2, index: 1);
    let note_3 = user_2
        .new_note_with_new_nonce(recipient: user_2, :token, amount: amount_1, index: 0);
...
fn test_use_note_same_amount() {
    ...
    user_1.open_channel_with_token_e2e(recipient: user_2, :token, subchannel_index: 0);
    let amount = 1;
    let note_1 = user_1.new_note_with_new_nonce(recipient: user_2, :token, :amount, index: 0);
    let note_2 = user_1.new_note_with_new_nonce(recipient: user_2, :token, :amount, index: 1);
    user_1.cheat_create_note_e2e(note: note_1);
    user_1.cheat_create_note_e2e(note: note_2);
    let channel_key = user_1.compute_channel_key(recipient: user_2);
    let use_note_input_1 = UseNoteInput {
...
fn test_set_viewing_key_multiple_times() {
    ...
    let expected_actions = [
        ServerAction::Write((public_key_storage_path_felt, user.public_key)),
...
fn test_compile_client_actions() {
    ...
    let expected_actions = [
        ServerAction::Write((public_key_storage_path_felt, user_1.public_key)),
        ServerAction::WritePrivateKey((enc_private_key_storage_path_felt, enc_private_key)),
    ]
        .span();
    assert_eq!(actions, expected_actions);
    ...
    let nonce = user_1.open_channel_e2e(recipient: user_2);
    let mut spy = spy_messages_to_l1();
    user_1
        .compile_client_actions(
            client_actions: [
    ...
    let amount = 100;
    let note = user_1.new_note_with_new_nonce(recipient: user_2, :token, :amount, index: 0);
    user_1.open_subchannel_e2e(recipient: user_2, :token, index: 0);
    let mut spy = spy_messages_to_l1();
    user_1
        .compile_client_actions(
    ...
    let actions = spy_messages_to_server_actions(ref :spy);
    let expected_enc_note = user_1
        .compute_enc_note(recipient: user_2, :token, index: 0, :amount, nonce: note.nonce);
    ...
    let create_note_input = user_2
        .new_note_with_new_nonce(recipient: user_1, :token, :amount, index: 0);
    user_2.open_channel_e2e(recipient: user_1);
    ...
    let expected_actions = array![
        ServerAction::WriteIfZero((nullifier_storage_path, true.into())),
        ServerAction::WriteIfZero((note_storage_path, expected_enc_note.enc_amount)),
    ]
        .span();
    assert_eq!(actions, expected_actions);
...
fn test_compile_client_actions_assertions() {
    ...
    let amount = 100;
    user.set_viewing_key_e2e();
    let note_1 = user.new_note_with_new_nonce(recipient: user, :token, :amount, index: 0);
    let note_1_path = UseNoteInput {
