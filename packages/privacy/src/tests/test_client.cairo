...
use privacy::utils::{decode_note_amount, encrypt_channel_info, is_canonical_key, unpacking};
...
fn test_create_note_at_existing_note_id(initial_is_open: bool, colliding_is_open: bool) {
    ...
    let result = if colliding_is_open {
        ...
        user_1.safe_create_open_note(create_note_input: open_note)
    } else {
        let enc_note = user_1
            .new_enc_note_with_generated_salt(
                recipient: user_2, :token_address, amount: 0, index: 0,
            );
        user_1.safe_create_enc_note(create_note_input: enc_note)
    };
    assert_panic_with_felt_error(:result, expected_error: errors::NON_ZERO_VALUE);
    ...
    if initial_is_open {
        ...
        let (note_id, _) = user_1.compute_open_note(create_note_input: open_note);
        depositor
            .fund_and_deposit_to_open_note(
    ...
    let channel_key = user_1.compute_channel_key(recipient: user_2);
    let use_note = UseNoteInput { channel_key, token: token_address, note_index: 0 };
    let transfer_note = user_2
        .new_enc_note_with_generated_salt(recipient: user_3, :token_address, :amount, index: 0);
    let actions = user_2
        .transfer(notes_to_use: [use_note].span(), notes_to_create: [transfer_note].span());
    test.privacy.execute_actions(:actions);
