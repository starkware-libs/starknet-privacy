...
/// # Algorithm
///
/// For each note index starting from `from_index`:
...
///   If `None`, discovers all notes from index 0.
///
/// # Returns
///
...
mod tests {
    ...
    async fn test_discover_no_notes() {
        ...
        let result = discover_notes(&backend, channel_key, token, None)
            .await
            .unwrap();
    ...
    async fn test_discover_notes_alice_self_channel() {
        ...
        assert_eq!(result.total_notes, result.notes.len() as u64);
    ...
    async fn test_discover_notes_incremental() {
        ...
        // First discovery
        let result1 = discover_notes(&backend, channel_key, token, None)
            .await
            .unwrap();
        assert!(!result1.notes.is_empty());

        // Incremental discovery starting from last index - should find 0 new notes
        let result2 = discover_notes(&backend, channel_key, token, Some(result1.total_notes - 1))
            .await
            .unwrap();
        assert_eq!(result2.notes.len(), 0);
        assert_eq!(result2.total_notes, result1.total_notes);
    }
}
