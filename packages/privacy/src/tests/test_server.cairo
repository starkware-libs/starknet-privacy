...
use privacy::tests::utils_for_tests::{
    ...
    PrivacyCfgTrait, Test, TestTrait, UserTrait, constants, deploy_mock_echo_with_salt,
    deploy_mock_reentrancy, deploy_mock_return_garbage, deploy_mock_return_trailing_garbage,
    deploy_mock_swap_executor, invoke_mock_swap_executor_input,
};
...
fn test_apply_emit_open_note_created() {
    ...
    let fill_actions = test
        .privacy
        .cheat_invoke_echo_input(:note_id, :token_addr, :amount)
        .into_server_actions();
...
fn test_deposit_to_open_note() {
    ...
    let create_note_input = user
        .new_open_note_with_generated_random(
            recipient: user, :token_addr, index: 0, depositor: echo_executor,
        );
    ...
    let (note_id, actions) = user.internal_create_and_fill_open_note(:create_note_input, :amount);
    ...
    assert_eq!(filled_note.depositor, echo_executor);
...
fn test_deposit_to_open_note_assertions() {
    ...
    let token_addr = token.contract_address();
    let echo_executor = test.privacy.echo_executor;
    let echo_executor_addr = echo_executor;
    ...
    let note_id_filled = user
        .create_and_fill_open_note_e2e(create_note_input: create_input, :amount, :token);
...
fn test_unfilled_open_notes() {
    ...
    // Catch TOO_MANY_OPEN_NOTES_FILLED: Invoke deposit without a matching EmitOpenNoteCreated.
    let create_input = user
        .new_open_note_with_generated_random(
            recipient: user, :token_addr, index: 0, depositor: echo_executor,
        );
    ...
    let result = test.privacy.safe_apply_actions(:actions);
    assert_panic_with_felt_error(
        :result, expected_error: internal_errors::TOO_MANY_OPEN_NOTES_FILLED,
    );
}
