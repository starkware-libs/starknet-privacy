...
#[generate_trait]
pub(crate) impl EkuboImpl of EkuboTrait {
    fn invoke_swap_external_input(
        self: @Ekubo, amount: u128, note_id: felt252,
    ) -> InvokeExternalInput {
        let calldata = build_ekubo_swap_helper_calldata(
            router_addr: *self.router,
            token_amount: TokenAmount {
                token: self.input_token.contract_address(),
                amount: ekubo::types::i129::i129 { mag: amount, sign: false },
            },
            pool_key: pool_key_for_tokens(
                self.input_token.contract_address(), self.output_token.contract_address(),
            ),
            minimum_received: 0,
            skip_ahead: 0,
            :note_id,
        );
        InvokeExternalInput { contract_address: *self.swap_helper, calldata: calldata.span() }
    }
}
