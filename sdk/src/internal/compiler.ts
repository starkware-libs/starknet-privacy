...
export class ActionCompiler {
  ...
  private transformToClientActions(
    ...
    assert(
      ...
      () => "At most one invoke-phase action (invoke / computeAndInvoke) per transaction"
    );
    ...
    if (actions.computeAndInvoke) {
      ...
      const compute_additional_data = CallData.compile(details.computeData ?? []).map(toBigInt);
      const invoke_additional_data = CallData.compile(details.invokeData ?? []).map(toBigInt);
