...
describe("getScreenedAddress", () => {
  ...
  it("screens nobody for an interaction that creates no open note", async () => {
    // The `CreateOpenNote` is here, so the empty note list is what the resolver reacts to rather
    // than the missing action. The policy is still read — the invoke is the transaction's open-note
    // depositor — but the pool assigns a subject only for an invoke that returns deposits, so it
    // asks for none and an attestation would revert with `UNEXPECTED_SCREENING`.
    const { screened, asked } = await subjectOf(
  ...
  it("screens nobody when a transaction creating no open note is otherwise underivable", async () => {
    // The note count is read before the felts the derivation needs, so an invoke depositing nothing
    // puts up nobody however unusable the rest of the call is — the pool reads those felts no more
    // than this does.
