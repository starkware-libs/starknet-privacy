...
use privacy::swap_executor::ekubo::EkuboSwapExecutor;
use privacy::swap_executor::ekubo::errors;
...
fn test_ekubo_privacy_invoke_basic() {
    ...
    // Balances after swap: in moved to mock, out moved to executor (approved for privacy).
    assert_eq!(input_token.balance_of(address: test.privacy.address), Zero::zero());
    ...
    assert_eq!(output_token.balance_of(address: test.privacy.address), Zero::zero());
    assert_eq!(output_token.balance_of(address: ekubo_executor.address), swap_amount.into());
    assert_eq!(output_token.balance_of(address: mock_router), Zero::zero());

    let out_erc20 = IERC20Dispatcher { contract_address: out_addr };
    assert_eq!(
        out_erc20.allowance(owner: ekubo_executor.address, spender: test.privacy.address),
        swap_amount.into(),
    );
}
...
fn test_ekubo_same_executor_different_pool() {
    ...
    // After first swap: executor has A + B (out approved for privacy), mock has A, C unchanged.
    assert_eq!(token_a.balance_of(address: test.privacy.address), Zero::zero());
    ...
    assert_eq!(token_b.balance_of(address: test.privacy.address), Zero::zero());
    assert_eq!(token_b.balance_of(address: ekubo_executor.address), swap_amount.into());
    assert_eq!(token_b.balance_of(address: mock_router), Zero::zero());
    assert_eq!(token_c.balance_of(address: test.privacy.address), Zero::zero());
    assert_eq!(token_c.balance_of(address: ekubo_executor.address), Zero::zero());
    assert_eq!(token_c.balance_of(address: mock_router), swap_amount.into());

    let token_b_erc20 = IERC20Dispatcher { contract_address: token_b.contract_address() };
    assert_eq!(
        token_b_erc20.allowance(owner: ekubo_executor.address, spender: test.privacy.address),
        swap_amount.into(),
    );
    ...
    // After second swap: executor has B + C (approved for privacy), mock has 2*A.
    assert_eq!(token_a.balance_of(address: test.privacy.address), Zero::zero());
    ...
    assert_eq!(token_b.balance_of(address: mock_router), Zero::zero());
    assert_eq!(token_c.balance_of(address: test.privacy.address), Zero::zero());
    assert_eq!(token_c.balance_of(address: ekubo_executor.address), swap_amount.into());
    assert_eq!(token_c.balance_of(address: mock_router), Zero::zero());

    let token_c_erc20 = IERC20Dispatcher { contract_address: token_c.contract_address() };
    assert_eq!(
        token_c_erc20.allowance(owner: ekubo_executor.address, spender: test.privacy.address),
        swap_amount.into(),
    );
}
