...
use privacy::tests::utils_for_tests::{
    EchoExecutorCfgTrait, PrivacyCfgTrait, Test, TestTrait, UserTrait,
};
...
fn test_e2e_client_actions_one_by_one() {
    ...
    // echo_executor, filled via echo executor in the same tx)
    ...
    out_token.supply(address: test.privacy.echo_executor.address, amount: amount_half);
    out_token
        .approve(
    ...
    assert_eq!(note_after_fill.depositor, test.privacy.echo_executor.address);
...
fn test_e2e_actions_twice() {
    ...
    token_1.supply(address: test.privacy.echo_executor.address, :amount);
    token_1
        .approve(
            owner: test.privacy.echo_executor.address,
            spender: test.privacy.address,
