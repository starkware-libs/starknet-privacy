...
function derivedShadowAccount(deployer: string): string {
  ...
  return "0x" + shadowAccountAddress(commitment, BigInt(deployer)).toString(16);
}
...
describe("getScreenedAddress", () => {
  ...
  it("screens any Delegated target's compute-invoke on its own shadow account", async () => {
    // The policy list is the authority: listing a target `Delegated` makes the derivation treat
    // it as a shadow account anonymizer, with the target itself as the deployer felt.
    const { screened } = await subjectOf(
