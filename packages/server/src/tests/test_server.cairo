...
fn test_create_note_zero_note_id() {
    let mut test: Test = Default::default();
    let note = test.new_encrypted_note();
    test.server.create_note(note: EncNote { id: Zero::zero(), enc_amount: note.enc_amount });
}
...
fn test_create_note_zero_enc_note_value() {
    let mut test: Test = Default::default();
    let note = test.new_encrypted_note();
    test.server.create_note(note: EncNote { id: note.id, enc_amount: Zero::zero() });
}
...
fn test_create_note_note_already_exists() {
    ...
    test.server.create_note(note: EncNote { id: note.id, enc_amount: diff_note.enc_amount });
}
