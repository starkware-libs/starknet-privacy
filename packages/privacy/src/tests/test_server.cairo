...
/// The addresses are read only for a `Delegated` depositor, and a depositor naming them under any
/// other policy is tolerated rather than rejected: the pool stops at the deposits and ignores what
/// follows. That tolerance is what lets the shadow account anonymizer — which names its address
/// on every interaction — fund open notes while still listed `Exempt`, and it is the exception
/// the `unread_return_data_tolerated` TODO exists to remove.
#[test]
...
/// funds nothing is never read for addresses. That is why a delegated depositor must append its
/// address span only when it returns deposits: with no deposits the pool stops at them, and a span
/// left behind — even an empty one — is trailing data it rejects.
#[test]
