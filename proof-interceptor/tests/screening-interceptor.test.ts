...
describe("ScreeningInterceptor", () => {
  ...
  describe("screening subjects beyond the depositor", () => {
    ...
    it("blocks the anonymizer's compute-invoke when it deposits into no open note", async () => {
      // The pool reverts a transaction whose open note goes undeposited, so the resolver refuses it
      // with its own reason instead of letting it through unscreened.
      const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
      const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
      const policyNode = await startDelegatedPolicyNode();
      const interceptor = new ScreeningInterceptor(
      ...
      expect(verdict.action).toBe("block");
      if (verdict.action === "block") {
        expect(verdict.reason).toBe("undeposited_open_notes");
      }
      await policyNode.close();
      logSpy.mockRestore();
      errorSpy.mockRestore();
