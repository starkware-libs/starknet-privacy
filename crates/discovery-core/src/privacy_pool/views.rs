...
use crate::storage_backend::{RawStorageAccess, StorageError, StorageValueWithBlock};
...
pub trait IViews: Send + Sync {
    ...
    ///
    /// Default delegates to `get_notes_batch` with `block_number = 0`.
    async fn get_notes_batch_with_block(
        &self,
        note_ids: &[Felt],
    ) -> Result<Vec<StorageValueWithBlock>, StorageError> {
        let note_values = self.get_notes_batch(note_ids).await?;
        Ok(note_values
            .into_iter()
...
impl<T: RawStorageAccess> IViews for T {
    ...
    async fn get_enc_private_key(&self, user_addr: Felt) -> Result<EncPrivateKey, StorageError> {
        ...
        let values = self.read_slots(slots.to_vec()).await?;
        Ok(EncPrivateKey {
            auditor_public_key: values[0],
            ephemeral_pubkey: values[1],
            enc_private_key: values[2],
        })
    }
