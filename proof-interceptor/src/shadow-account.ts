...
import { normalizeFelt, type PoolCallActions } from "./pool-transaction.js";
 ...
 * runs none. It is the address to screen for the open notes that interaction funds.
 *
...
function anonymizerArgumentTypes(name: string): string[] {
  for (const item of ShadowAccountAnonymizerABI) {
    if (item.type !== "interface") continue;
    for (const entry of item.items) {
      if (entry.name === name) {
        return entry.inputs.slice(1).map((input) => input.type);
      }
    }
  }
