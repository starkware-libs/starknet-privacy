...
fn test_note_zero() {
    ...
    let non_zero_note = Note { enc_value, token: Zero::zero() };
    assert!(non_zero_note.is_non_zero());
    assert!(!non_zero_note.is_zero());
    let non_zero_note = Note { enc_value: Zero::zero(), token };
    assert!(non_zero_note.is_non_zero());
    assert!(!non_zero_note.is_zero());
}
