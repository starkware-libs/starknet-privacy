...
import { classifyTransaction } from "../../src/internal/action-classifier.js";
import type { HistoryTransaction } from "../../src/internal/history.js";
...
describe("classifyTransaction", () => {
  ...
  it("classifies self_channel-only notes as transferSelf (sweep/spray)", () => {
    const { actions } = classifyTransaction(
      emptyTransaction({
        notes: [
          {
    ...
    expect(actions).toEqual([{ type: "transferSelf", token: STRK, amount: 50n, noteCount: 1 }]);
  });
  ...
  it("aggregates multiple self_channel notes by token for transferSelf", () => {
    const { actions } = classifyTransaction(
      emptyTransaction({
        notes: [
          {
    ...
    expect(actions).toEqual([{ type: "transferSelf", token: STRK, amount: 50n, noteCount: 2 }]);
  });

  it("produces separate transferSelf actions per token", () => {
    const { actions } = classifyTransaction(
      emptyTransaction({
        notes: [
          {
    ...
    expect(actions).toEqual([
      { type: "transferSelf", token: STRK, amount: 30n, noteCount: 1 },
      { type: "transferSelf", token: ETH, amount: 10n, noteCount: 1 },
    ]);
  });

  it("does not produce transferSelf when self_channel notes coexist with deposits", () => {
    const { actions } = classifyTransaction(
      emptyTransaction({
        deposits: [{ fromAddress: ALICE, token: STRK, amount: 100n }],
        notes: [
  ...
  it("does not produce transferSelf when self_channel notes coexist with outgoing notes", () => {
    const { actions } = classifyTransaction(
      emptyTransaction({
        notes: [
          {
