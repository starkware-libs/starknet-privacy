...
describe("Encryption Compatibility with Cairo", () => {
  ...
  describe("Channel Info Encryption (ECDH)", () => {
    ...
    it("encrypted channel key matches Cairo", () => {
      ...
      expect(result.encChannelKey.toString(16)).toBe(BigInt(outputs.encChannelKey).toString(16));
    });
    ...
    it("encrypted sender addr matches Cairo", () => {
      ...
      expect(result.encSenderAddr.toString(16)).toBe(
    ...
    it("decrypt channel info recovers original values", () => {
      const encrypted = {
        ephemeralPubkey: BigInt(outputs.encChannelEphemeralPubkey),
        encChannelKey: BigInt(outputs.encChannelKey),
        encSenderAddr: BigInt(outputs.encChannelSenderAddr),
      };
