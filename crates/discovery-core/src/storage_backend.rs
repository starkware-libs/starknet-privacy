...
/// A storage value paired with the block number at which it was last updated.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct StorageValueWithBlock {
    pub value: Felt,
...
pub trait RawStorageAccess: Send + Sync {
    ...
    ///
    /// Default implementation delegates to `read_slots` with block_number=0.
    async fn read_slots_with_block(
        &self,
        slots: Vec<Felt>,
    ) -> Result<Vec<StorageValueWithBlock>, StorageError> {
        let values = self.read_slots(slots).await?;
        Ok(values
            .into_iter()
            .map(|value| StorageValueWithBlock {
                value,
                block_number: 0,
            })
            .collect())
...
pub struct MockBackend {
    ...
    block_numbers: HashMap<Felt, u64>,
}
...
impl MockBackend {
    ...
    pub fn insert_with_block(&mut self, slot: Felt, value: Felt, block_number: u64) {
        self.slots.insert(slot, value);
        self.block_numbers.insert(slot, block_number);
    }
}
...
impl RawStorageAccess for MockBackend {
    ...
    async fn read_slots_with_block(
        ...
        Ok(slots
            .iter()
            .map(|s| StorageValueWithBlock {
                value: self.slots.get(s).copied().unwrap_or(Felt::ZERO),
                block_number: self.block_numbers.get(s).copied().unwrap_or(0),
            })
            .collect())
    }
}
...
mod tests {
    ...
    async fn test_read_slots_with_block_default() {
        ...
        assert_eq!(results[0].block_number, 0);
        assert_eq!(results[1].value, Felt::ZERO);
        assert_eq!(results[1].block_number, 0);
    }
    ...
    async fn test_read_slots_with_block_with_data() {
        ...
        assert_eq!(results[1].block_number, 200);
        assert_eq!(results[2].value, Felt::ZERO);
        assert_eq!(results[2].block_number, 0);
    }
}
