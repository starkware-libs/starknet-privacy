...
describe("dapp client: Eth712Account (EVM) deposit on devnet", () => {
  ...
  beforeAll(async () => {
    ...
    account = await deployEth712Account(env.env.admin, env.env.node, EVM_KEY);
  }, E2E_TIMEOUTS.hook);
    ...
    return {
      ...
      async executeTransaction(
        ...
        if (execute.kind === "invokeAndApplyAction" && outsideExecution) {
          ...
          await node.waitForTransaction(relay.transaction_hash);
        }
  ...
  const poolStrkBalance = (): Promise<bigint> =>
    tokenBalance(env.env.node, env.env.strk, env.env.privacy.address);
  ...
  it(
    ...
    async () => {
      ...
      await node.waitForTransaction(fund.transaction_hash);
  ...
  it(
    ...
    async () => {
      ...
      const { strk, alice, node } = env.env;
      const withdrawn = KEPT - FEE;
      const aliceStrk = (): Promise<bigint> =>
        tokenBalance(node, strk, alice.address);
