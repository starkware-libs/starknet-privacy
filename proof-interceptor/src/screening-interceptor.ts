 ...
 * error `data`, so they must not reveal an address. Keying the record by the kinds themselves makes
 * a new one a compile error rather than a kind that silently falls through to being screened.
 */
...
> = {
  ...
  unimplementedDelegate: "delegated_screening_unimplemented",
