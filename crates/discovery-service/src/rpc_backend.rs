...
use discovery_core::storage_backend::{
    RawStorageAccess, StorageBackend, StorageError, StorageSnapshot, StorageValueWithBlock,
};
use starknet_core::types::{
    requests::GetStorageAtRequest, BlockId, BlockTag, Felt, MaybePreConfirmedBlockWithTxHashes,
    StarknetError, StorageResponseFlag,
};
...
impl RpcSnapshot {
    ...
    async fn batch_read_with_block(
        ...
        responses
            .into_iter()
            .map(|resp| match resp {
                ProviderResponseData::GetStorageAtWithFlags(result) => Ok(StorageValueWithBlock {
                    value: result.value,
                    block_number: result.last_update_block,
                }),
                _ => Err(RpcBackendError::UnexpectedResponseType.into()),
            })
            .collect()
