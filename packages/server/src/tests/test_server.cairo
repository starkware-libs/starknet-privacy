...
fn test_get_note() {
    let mut test: Test = Default::default();
    let note = test.new_encrypted_note();
    assert_eq!(test.server.get_note(note_id: note.id), Zero::zero());
    test.server.create_note(:note);
    assert_eq!(test.server.get_note(note_id: note.id), note.encrypted_amount);
}
...
fn test_create_note() {
    let mut test: Test = Default::default();
    let note = test.new_encrypted_note();
    test.server.create_note(note: note);
    assert_eq!(test.server.get_note(note_id: note.id), note.encrypted_amount);
}
...
fn test_create_note_twice() {
    let mut test: Test = Default::default();
    let note_1 = test.new_encrypted_note();
    test.server.create_note(note: note_1);
    let note_2 = test.new_encrypted_note();
    test.server.create_note(note: note_2);
    assert_eq!(test.server.get_note(note_id: note_1.id), note_1.encrypted_amount);
    assert_eq!(test.server.get_note(note_id: note_2.id), note_2.encrypted_amount);
}
...
fn test_create_note_zero_note_id() {
    ...
    test
        .server
        .create_note(
            note: EncryptedNote { id: Zero::zero(), encrypted_amount: note.encrypted_amount },
        );
}
...
fn test_create_note_zero_enc_note_value() {
    let mut test: Test = Default::default();
    let note = test.new_encrypted_note();
    test.server.create_note(note: EncryptedNote { id: note.id, encrypted_amount: Zero::zero() });
}
...
fn test_create_note_note_already_exists() {
    ...
    let note = test.new_encrypted_note();
    test.server.create_note(:note);
    let diff_note = test.new_encrypted_note();
    test
        .server
        .create_note(
            note: EncryptedNote { id: note.id, encrypted_amount: diff_note.encrypted_amount },
        );
}
