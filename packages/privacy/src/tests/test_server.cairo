...
fn test_combined_regular_and_open_note_deposits() {
    ...
    // Open-note deposit: the depositor is the Invoke target (echo_executor), which carries no
    // attestation requirement of its own here.
    let mut user = test.new_user();
    user.set_viewing_key_e2e();
    user.open_channel_with_token_e2e(recipient: user, :token_addr, outgoing_channel_index: 0);
    let create_note_input = user
    ...
    // Both deposits land under the regular depositor's attestation: the open-note depositor's
    // policy adds no requirement of its own.
    test
        .privacy
        .apply_actions_screened(
