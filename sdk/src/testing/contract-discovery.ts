...
export class ContractDiscoveryProvider extends AbstractDiscoveryProvider {
  ...
  async discoverNotes(
    ...
    for (c = cursor.channelKeyIndex ?? 0; c < nc; c++) {
      ...
      const channel = encryptions.decryptChannelInfo(
        {
          ephemeralPubkey: toBigInt(encryptedChannel.ephemeral_pubkey),
          encChannelKey: toBigInt(encryptedChannel.enc_channel_key),
          encSenderAddr: toBigInt(encryptedChannel.enc_sender_addr),
        },
        toBigInt(viewingKey)
      );
    ...
    for (const [sender, senderCursor] of cursor.senders) {
      ...
      for (k = senderCursor.subchannelKeyIndex; ; k++) {
        ...
        const { token } = encryptions.decryptSubchannelInfo(
          { salt: toBigInt(encSubchannel.salt), enc_token: toBigInt(encSubchannel.enc_token) },
          channelKey,
          k
        );
  ...
  async discoverChannels(
    ...
    for (const recipient of recipients) {
      ...
      // Use hardcoded ephemeral secret (2n) to match channel - this is temporary until forward tracing
      const encryptedChannelInfo = encryptions.encryptChannelInfo(
        2n,
        toBigInt(publicKey),
        channelKey,
        toBigInt(address)
      );
      const nc = await this.pool.get_num_of_channels(hex(recipient));
      for (let c = 0; c < nc; c++) {
        const other = await this.pool.get_channel_info(hex(recipient), c);
        if (toBigInt(other.enc_channel_key) === encryptedChannelInfo.encChannelKey) {
