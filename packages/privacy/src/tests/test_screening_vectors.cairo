...
use crate::snip12::{
    DepositorValidation, ScreeningAttestation, compute_screening_message_hash, is_screening_attestation_valid,
};
...
fn test_committed_screening_vectors_validate() {
    for vector in screening_vectors() {
        ...
        assert_eq!(
            compute_screening_message_hash(@validation, vector.signer_public_key), vector.message_hash,
        );
