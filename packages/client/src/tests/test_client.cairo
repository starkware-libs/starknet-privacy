...
use client::utils::{
    encrypt_channel_info, encrypt_note_amount, get_channel_id, get_channel_key, get_note_id,
    is_canonical_key,
};
...
fn test_transfer_to_self() {
    ...
    let result = user
        .transfer(
            notes_to_use: [NotePath { channel_index: 0, note_index: 0 },].span(),
            notes_to_create: [NewNote { recipient_addr: user.address, token, amount: 1, index: 0 }]
                .span(),
        );
    ...
    let result = user_1
        .transfer(
            ...
            notes_to_create: [
                NewNote { recipient_addr: user_2.address, token, amount: 1, index: 0 },
                NewNote { recipient_addr: user_2.address, token, amount: 1, index: 0 },
                NewNote { recipient_addr: user_3.address, token, amount: 8, index: 0 },
            ]
                .span(),
        );
    let expected_result = ([].span(), [].span());
...
fn test_create_note() {
    ...
    let note_id = get_note_id(:channel_key, index: note_index, public_key: user_2.public_key);
...
fn test_create_note_self_note() {
    ...
    let note_id = get_note_id(:channel_key, index: note_index, public_key: user.public_key);
    let enc_amount = encrypt_note_amount(:channel_key, :amount);
    assert_eq!(enc_note.id, note_id);
    assert_eq!(enc_note.enc_amount, enc_amount);
}
...
fn test_create_note_twice() {
    ...
    let note_id_1 = get_note_id(:channel_key, index: note_index_1, public_key: user_2.public_key);
    let note_id_2 = get_note_id(:channel_key, index: note_index_2, public_key: user_2.public_key);
