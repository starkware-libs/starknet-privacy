...
fn test_execute_write_once_open_note() {
    // Test that server correctly writes both packed_value and token for open notes.
    let mut test: Test = Default::default();
    ...
    // Compute the storage address for the note.
    let storage_address = map_entry_address(
        map_selector: selector!("notes"), keys: [note_id].span(),
    );

    // Manually create WriteOnce action with both packed_value and token.
    let actions = [
        ServerAction::WriteOnce(
            WriteOnceInput {
