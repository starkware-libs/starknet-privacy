...
use privacy::objects::{
    EncChannelInfo, EncOutgoingChannelInfo, EncPrivateKey, EncSubchannelInfo, TokenBalances,
    TokenBalancesTrait,
};
...
pub(crate) impl UserImpl of UserTrait {
    ...
    fn open_channel(self: @User, recipient: User, index: usize, random: felt252, salt: felt252) {
    ...
    fn open_channel_e2e(ref self: User, recipient: User, index: usize) -> (felt252, felt252) {
        ...
        let salt = self.get_salt().into();
        let mut spy = spy_messages_to_l1();
        self.open_channel(:recipient, :index, :random, :salt);
