...
pub(crate) impl UserImpl of UserTrait {
    ...
    fn create_note_e2e(self: @User, note: NewNote) {
        ...
        self.privacy.mock_create_note(note: enc_note);
    }
    ...
    /// Mock function to deposit in the server side.
    fn mock_deposit_server(self: @User, token: Token, amount: u128, note: EncNote) {
    ...
    /// Mock function to withdraw in the server side.
    fn mock_withdraw_server(
        self: @User,
...
pub(crate) impl PrivacyCfgImpl of PrivacyCfgTrait {
    /// Mock function to open a channel in the server side.
    fn mock_open_channel(
        self: @PrivacyCfg,
        recipient_addr: ContractAddress,
    ...
    /// Mock function to create a note in the server side.
    fn mock_create_note(self: @PrivacyCfg, note: EncNote) {
    ...
    /// Mock function to use a note in the server side.
    fn mock_use_note(self: @PrivacyCfg, nullifier: felt252) {
        let storage_path_felt = map_entry_address(
            map_selector: selector!("nullifiers"), keys: [nullifier].span(),
        );
        self
...
pub(crate) fn deploy_privacy() -> PrivacyCfg {
    ...
    PrivacyCfg {
        ...
        server: IServerDispatcher { contract_address: contract_address },
        safe_server: IServerSafeDispatcher { contract_address: contract_address },
        client: IClientDispatcher { contract_address: contract_address },
        safe_client: IClientSafeDispatcher { contract_address: contract_address },
        views: IViewsDispatcher { contract_address: contract_address },
        safe_views: IViewsSafeDispatcher { contract_address: contract_address },
    }
}
