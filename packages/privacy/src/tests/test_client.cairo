...
fn test_internal_actions() {
    ...
    // First, execute the enc note actions so that index 0 exists (required for index 1).
    let create_open_note_input = user_1
        .new_open_note(recipient: user_2, token: token_address, index: note_index);
    let open_note_actions = user_1
        .internal_create_open_note(create_note_input: create_open_note_input);
    assert_eq!(open_note_actions, create_open_note_input.into_server_actions(user: user_1));
