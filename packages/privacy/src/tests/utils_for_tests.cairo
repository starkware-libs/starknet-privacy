...
pub(crate) impl UserImpl of UserTrait {
    ...
    fn open_subchannel(
        self: @User, recipient: User, token_address: ContractAddress, index: usize, nonce: felt252,
    ) {
    ...
    /// Returns (nonce, output) where output is the output of `open_subchannel`.
    fn internal_open_subchannel_with_new_nonce(
        ref self: User, recipient: User, token_address: ContractAddress, index: usize,
    ) -> (felt252, Span<ServerAction>) {
        let nonce = self.get_nonce().into();
        let output = self.internal_open_subchannel(:recipient, :token_address, :index, :nonce);
        (nonce, output)
    }

    /// Returns the nonce generated by the user for the subchannel opening.
    fn open_subchannel_e2e(
        ref self: User, recipient: User, token_address: ContractAddress, index: usize,
    ) -> felt252 {
        let nonce = self.get_nonce().into();
        let mut spy = spy_messages_to_l1();
        self.open_subchannel(:recipient, :token_address, :index, :nonce);
        let actions = spy_messages_to_server_actions(ref :spy);
        self.privacy.revert_actions_for_testing(:actions);
        self.privacy.server.execute_actions(:actions);
        nonce
    ...
    fn compute_enc_subchannel_info(
        self: @User, recipient: User, token_address: ContractAddress, nonce: felt252,
    ) -> EncSubchannelInfo {
        let channel_key = self.compute_channel_key(:recipient);
        encrypt_subchannel_info(:channel_key, token: token_address, :nonce)
    }
    ...
    fn compute_enc_note(
        ...
        let enc_amount = encrypt_note_amount(:channel_key, :nonce, :amount);
    ...
    fn new_note_with_new_nonce(
        ref self: User, recipient: User, token_address: ContractAddress, amount: u128, index: usize,
    ) -> CreateNoteInput {
        let nonce = self.get_nonce().into();
        self.new_note(:recipient, :token_address, :amount, :index, :nonce)
    }
...
pub(crate) impl TestImpl of TestTrait {
    ...
    fn mock_new_subchannel(ref self: Test) -> (felt252, felt252, EncSubchannelInfo) {
        ...
        let enc_subchannel_info = EncSubchannelInfo {
            nonce: self.nonce.into(), enc_token: 'ENC_TOKEN' + self.nonce.into(),
        };
...
pub(crate) fn decrypt_subchannel_token(
    ...
    let token = enc_subchannel_info.enc_token
        - compute_enc_token_hash(:channel_key, nonce: enc_subchannel_info.nonce);
