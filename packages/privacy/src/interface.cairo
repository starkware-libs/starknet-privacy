...
pub trait IServer<T> {
    ...
    ///   [`NoteUsed`](privacy::events::NoteUsed) event.
    ///   - [`Invoke`](privacy::actions::ServerAction::Invoke): Invoke an external contract. The
    ///   invoked method should return a
    ///   `Span<`[`DepositToOpenNoteInput`](privacy::objects::DepositToOpenNoteInput)`>` (which may
    ///   be empty).
    ///
    /// #### Returns
    /// None
    ...
    /// when `deposit_to_open_note` is executed for it.
    ///
    /// #### Reverts
    ...
    ///
    /// **Errors for [`DepositToOpenNoteInput`](privacy::objects::DepositToOpenNoteInput)
    /// (triggered by [`Invoke`](privacy::actions::ServerAction::Invoke)):**
    /// - [`ZERO_DEPOSITOR`](privacy::errors::ZERO_DEPOSITOR): Thrown if `depositor` is zero.
    ...
    /// - `INSUFFICIENT_BALANCE`: Thrown if the depositor has insufficient token balance (from ERC20
    /// contract).
