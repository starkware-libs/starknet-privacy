...
pub mod Client {
    ...
    pub(crate) impl ClientInternalImpl of ClientInternalTrait {
        ...
        /// Stores a note in storage (server-side operation).
        fn _create_note(ref self: ContractState, note: EncNote) {
            ...
            assert(self.get_note(note_id: note.id).is_zero(), errors::NOTE_ALREADY_EXISTS);

            // Write note to storage.
            self.notes.write(note.id, note.enc_amount);
        }

        /// Marks a nullifier as used (server-side operation).
        fn _use_note(ref self: ContractState, nullifier: felt252) {
            ...
            assert(!self.nullifier_exists(:nullifier), errors::NULLIFIER_ALREADY_EXISTS);
