...
pub(crate) impl UserImpl of UserTrait {
    ...
    fn open_subchannel(
        self: @User, recipient: User, token: ContractAddress, index: usize, nonce: felt252,
    ) {
    ...
    fn internal_open_subchannel_with_new_nonce(
        ...
        let output = self.internal_open_subchannel(:recipient, :token, :index, :nonce);
        (nonce, output)
    }
    ...
    fn open_subchannel_e2e(
        ...
        let nonce = self.get_nonce().into();
        let mut spy = spy_messages_to_l1();
        self.open_subchannel(:recipient, :token, :index, :nonce);
        let actions = spy_messages_to_server_actions(ref :spy);
        self.privacy.server.execute_actions(:actions);
        nonce
    }
    ...
    fn compute_enc_subchannel_info(
        self: @User, recipient: User, token: ContractAddress, nonce: felt252,
    ) -> EncSubchannelInfo {
        let channel_key = self.compute_channel_key(:recipient);
        encrypt_subchannel_info(:channel_key, :token, :nonce)
    }
    ...
    fn new_note_with_new_nonce(
        ...
        let nonce = self.get_nonce().into();
        self.new_note(:recipient, :token, :amount, :index, :nonce)
    }
