...
pub mod Privacy {
    ...
    use privacy::utils::{
        ...
        require_screening_subject, send_message_to_server, storage_path_to_felt252,
    ...
    pub impl ServerImpl of IServer<ContractState> {
        fn apply_actions(
            ...
            if let Some(screening_subject) = self._apply_actions(:actions) {
                // The actions require screening: the attestation must cover the screening subject.
                self
                    ._verify_screening(
                        screening.expect(errors::SCREENING_REQUIRED), :screening_subject,
                    );
            } else {
                // Nothing in the actions requires screening.
    ...
    pub impl ServerInternalImpl of ServerInternalTrait {
            ...
            for action in actions {
                match *action {
                    ...
                    ServerAction::TransferFrom(input) => {
                        // A regular-pool deposit requires screening of its depositor, so every
                        // `TransferFrom` in the tx must share one `from_addr`.
                        require_screening_subject(
                            ref required_screening_subject, screening_subject: input.from_addr,
                        );
                    ...
                    ServerAction::Invoke(input) => {
                        self
                            ._apply_invoke_and_deposits(
                                ...
                                ref :required_screening_subject,
                            );
                    },
        ...
        /// deposits are associated with after them, and only then is anything past the deposits
        /// accepted.
        ...
        fn _apply_invoke_and_deposits(
            ...
            ref required_screening_subject: Option<ContractAddress>,
        ) {
            ...
            let mut associated_addresses_read = false;
            if !deposits.is_empty() {
                match self.open_note_depositor_screening_policies.read(contract_address) {
                    ...
                    // own address.
                    OpenNoteScreeningPolicy::Required => require_screening_subject(
                        ref required_screening_subject, screening_subject: contract_address,
                    ),
                    OpenNoteScreeningPolicy::Exempt => {},
                    // A delegated depositor names the addresses its deposits are associated with
                    // —
                    // the users it funded them for, rather than its own address — in the return
                    // data following the deposits. Only a compute-invoke is delegated; under a
                    // plain invoke the depositor is exempt and returns nothing further.
                    OpenNoteScreeningPolicy::Delegated => {
                        if selector == INVOKE_WITH_COMPUTATION_SELECTOR {
                            ...
                            for associated_address in associated_addresses {
                                require_screening_subject(
                                    ref required_screening_subject,
                                    screening_subject: *associated_address,
                                );
                            }
                            associated_addresses_read = true;
                        }
                    },
                }
            ...
            // Data past the deposits is rejected, with one temporary exception: a depositor that
            // returned deposits under a policy reading no addresses may leave it behind. The shadow
            // account anonymizer names its associated addresses on every interaction, so without
            // this it could not fund an open note at all while listed anything but `Delegated`.
            // TODO(Avi): drop `unread_return_data_tolerated` once every environment running the
            // anonymizer lists it `Delegated`. A depositor returning data the pool does not read is
            // a misconfiguration worth failing on, and until then this hides it.
            let unread_return_data_tolerated = !deposits.is_empty() && !associated_addresses_read;
            assert(
                return_data.is_empty() || unread_return_data_tolerated,
                errors::INVALID_INVOKE_RETURN_DATA,
            );
        }
