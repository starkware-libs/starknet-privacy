...
- `IndexerDiscoveryProvider.discoverChannels(..., "total-only")` now paginates `/v1/sync/outgoing_state` until it can determine the channel count, either from `total_n_channels` or from a completed discovery cursor, instead of assuming the count is present on the first page.
