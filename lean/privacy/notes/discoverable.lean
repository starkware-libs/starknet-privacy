...
theorem note_exists_implies_i₀
    ...
    ∃ sn: ScannedNote,
      ...
      sn ∈ scan_notes_for_channel_i₀ (.from rm) sn.c sn.token sn.i₀ ∧
      sn.i₀ < crypto.MAX_I₀ ∧
      subchannel_exists crypto rm sn.c sn.token := by
  revert rm
  apply ReachableMemory.induction
  case inv₀ => simp [note_exists, ReachableMemory.m]
...
theorem note_exists_implies_for_recipient
  ...
  obtain ⟨addralice, kalice, addrbob, kbob, h'⟩ := channels_are_discoverable h_channel_exists

  use addrbob, kbob, sn
  rw [scan_notes_for_recipient']

  exact ⟨⟨h'.2, h_token, note_id_in_scan⟩, h_note_id, h_channel_exists, ⟨addralice, kalice, h'.1⟩⟩
