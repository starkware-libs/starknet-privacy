...
pub trait IServer<T> {
    ...
    ///
    /// #### Parameters
    /// - `owner` (`ContractAddress`): The address of the owner of the funds. Must not be zero.
    ...
    /// - The owner must have approved the contract to spend the amount.
    /// - The owner must have enough balance.
    ///
    /// #### Events Emitted
    /// - [`Deposit`](server::events::Deposit): Emitted when the deposit is successful.
    ///
    /// #### Reverts
    /// - [`ZERO_OWNER`](server::errors::ZERO_OWNER): Thrown if `owner` is zero.
    ...
    /// exists.
    /// - [`INSUFFICIENT_ALLOWANCE`](server::errors::INSUFFICIENT_ALLOWANCE): Thrown if the
    /// allowance is insufficient.
    /// - [`INSUFFICIENT_BALANCE`](server::errors::INSUFFICIENT_BALANCE): Thrown if the balance is
    /// insufficient.
    ///
    /// #### Access Control
    /// - TBD
    fn deposit(
        ref self: T, owner: ContractAddress, token: ContractAddress, amount: u128, note: EncNote,
    );
}
