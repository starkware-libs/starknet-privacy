...
fn _set_privacy_roles(contract: ContractAddress, roles: Roles) -> Roles {
    ...
    // Each grant is caller-gated on the role's admin: SecurityAgent/SecurityGovernor are
    // administered by SecurityAdmin and AppRoleAdmin by GovernanceAdmin (both held by
    // governance_admin), while AppGovernor is administered by AppRoleAdmin. AppRoleAdmin is
    // therefore granted before AppGovernor.
