...
export class ActionCompiler {
  ...
  private transformToClientActions(
    ...
    if (actions.computeAndInvoke) {
      ...
      const compute_additional_data = CallData.compile(details.computeData ?? []).map((value) =>
        toBigInt(value)
      );
      const invoke_additional_data = CallData.compile(details.invokeData ?? []).map((value) =>
        toBigInt(value)
      );
