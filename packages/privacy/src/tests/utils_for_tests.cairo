...
impl CloneUserImpl of Clone<User> {
...
pub(crate) impl UserFlowImpl of UserFlowTrait {
    fn open_channel_flow_actions(self: @User, ref recipient: User) -> Span<ClientAction> {
        ...
        if (!self.is_registered()) {
            client_actions
                .append_span(self.set_viewing_key_actions(random: recipient.get_random().into()));
        }

        client_actions
            .append_span(
                self
                    .open_channel_actions(
                        recipient: @recipient, random: recipient.get_random().into(),
                    ),
            );
