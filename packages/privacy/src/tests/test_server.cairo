...
fn test_deposit_to_open_note() {
    ...
    let (note_id, actions) = user
        .internal_create_and_fill_open_note(executor: @echo_executor, :create_note_input, :amount);
...
fn test_unfilled_open_notes() {
    ...
    // Catch OPEN_NOTE_COUNT_MISMATCH: EmitOpenNoteCreated without a matching Invoke deposit.
    let create_input = user
        .new_open_note_with_generated_random(
            recipient: user, :token_addr, index: 0, depositor: echo_executor.address,
        );
    ...
    let result = test.privacy.safe_apply_actions(:actions);
    assert_panic_with_felt_error(:result, expected_error: errors::OPEN_NOTE_COUNT_MISMATCH);

    // Catch OPEN_NOTE_COUNT_MISMATCH: Invoke deposit without a matching EmitOpenNoteCreated.
    let create_input = user
        .new_open_note_with_generated_random(
            recipient: user, :token_addr, index: 0, depositor: echo_executor.address,
        );
