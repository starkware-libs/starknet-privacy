...
pub mod SwapExecutor {
    ...
    pub impl SwapExecutorImpl of ISwapExecutor<ContractState> {
        fn swap_and_deposit(
            ...
            let received_amount = execute_swap(
            ...
            if received_amount > 0 {
                deposit_to_open_note(
                    :privacy_pool, :output_token_dispatcher, :received_amount, :note_id,
                );
            }
        }
    }
    ...
    fn deposit_to_open_note(
        ...
        output_token_dispatcher.approve(spender: privacy_pool, amount: received_amount);

        // Deposit to open note.
        IServerDispatcher { contract_address: privacy_pool }
            .execute_actions(
                [
                    ServerAction::DepositToOpenNote(
                        DepositToOpenNoteInput {
                            note_id, amount: received_amount.try_into().unwrap(),
                        },
                    ),
                ]
                    .span(),
