...
pub trait IClient<T> {
    ...
    /// `note.recipient_addr` is considered the owner, and used as the address to transfer the
    /// deposit from.
    ...
    /// - `owner_private_key` matches the `note.recipient_addr`'s public key.
    /// - `note.index` is sequential within the channel.
    ///
    /// #### Events Emitted
    /// None
    ///
    ...
    /// `note.recipient_addr` with `token` doesn't exist with the given `owner_private_key`.
