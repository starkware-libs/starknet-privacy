...
/// Decoded fields for a Cairo `NoteCreated` event.
/// Keys = `[selector, note_id]`, data = `[packed_value]`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NoteCreatedRawEvent {
    pub note_id: Felt,
...
pub enum PrivacyPoolEventContent {
    ...
    NoteCreated(NoteCreatedRawEvent),
...
pub trait IEvents: Send + Sync {
    ...
    ///
    /// Parses all known event types (Deposit, Withdrawal, NoteCreated, OpenNoteDeposited).
...
impl TryFrom<EmittedEvent> for PrivacyPoolEvent {
    ...
    fn try_from(event: EmittedEvent) -> Result<Self, Self::Error> {
        ...
        let content = if selector == starknet_keccak(b"Deposit") {
...
impl<T: RawEventAccess> IEvents for T {
    ...
    async fn get_withdrawal_events(
        ...
        let withdrawal_selector = starknet_keccak(b"Withdrawal");
        let raw_events = self
            .get_events(
    ...
    async fn get_block_events_returns_all_event_types() {
        ...
        let backend = MockEventBackend::new(vec![
            ...
            mock_event(
                10,
                TX_HASH,
                vec![note_created_selector(), note_id],
                vec![packed_value],
            ),
        ...
        assert!(matches!(
            events[1].content,
            PrivacyPoolEventContent::NoteCreated(_)
        ));
        assert!(matches!(
            events[2].content,
