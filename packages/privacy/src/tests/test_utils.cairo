...
use privacy::utils::{
    ...
    encrypt_subchannel_info, encrypt_user_addr, open_note, pack, set_uniq_address,
...
fn test_compute_message_hash_depends_on_class_hash() {
    ...
    let real_class_hash = get_class_hash(:contract_address);
    ...
    let mut real_payload = array![];
    real_class_hash.serialize(ref real_payload);
    actions.serialize(ref real_payload);
...
#[test]
fn test_set_uniq_address_records_first_screening_subject() {
    ...
    set_uniq_address(ref screening_subject, reference: address);
    assert_eq!(screening_subject, Some(address));
}

#[test]
fn test_set_uniq_address_dedups_same_screening_subject() {
    let address: ContractAddress = hash(['SCREENING_SUBJECT'].span()).try_into().unwrap();
    let mut screening_subject: Option<ContractAddress> = None;
    set_uniq_address(ref screening_subject, reference: address);
    set_uniq_address(ref screening_subject, reference: address);
...
fn test_set_uniq_address_rejects_the_zero_address() {
    let mut screening_subject: Option<ContractAddress> = None;
    set_uniq_address(ref screening_subject, reference: Zero::zero());
}

#[test]
#[should_panic(expected: 'MULTIPLE_SCREENING_SUBJECTS')]
fn test_set_uniq_address_rejects_distinct_screening_subject() {
    let address: ContractAddress = hash(['SCREENING_SUBJECT'].span()).try_into().unwrap();
    let other_address: ContractAddress = hash(['OTHER_SCREENING_SUBJECT'].span())
        .try_into()
        .unwrap();
    let mut screening_subject: Option<ContractAddress> = None;
    set_uniq_address(ref screening_subject, reference: address);
    set_uniq_address(ref screening_subject, reference: other_address);
}
