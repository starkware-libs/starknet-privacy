...
// (The Python signer produces the same messageHash + signerPublicKey for the same inputs; its
// `(r, s)` differs because STARK ECDSA uses a non-deterministic nonce.) To regenerate:
//   cd scripts/address_validation_signer/ts
//   npm run sign -- --signer-private-key 0xCAFEBABE --depositor 0x1234 \
//                   --issued-at 1700000000 --chain-id TEST
const REF_DEPOSITOR: felt252 = 0x1234;
const REF_ISSUED_AT: u64 = 1700000000;
const REF_SIGNER_PUBLIC_KEY: felt252 =
    0x7f1fff02c3801d82c9b233faefb45f99631c4c85a3325b785884e38ee56ae46;
const REF_MESSAGE_HASH: felt252 = 0x67b37a2ba91ebf69df6da224d2dfd87b8839c2f7143f2e9fc1bfe2fc7fb746;
const REF_R: felt252 = 0x22ed8b6e747fa2178001e036376d0771d87080ac911888b67227804041ab6da;
const REF_S: felt252 = 0x11926cad35e3531bd146b6cc8ad8e3795e7011bd1cdc6b8fe2ded0f2038f6fc;
...
#[test]
fn test_externally_signed_vector_validates() {
    setup_chain_id();
    let validation = DepositorValidation {
        depositor: REF_DEPOSITOR.try_into().unwrap(), issued_at: REF_ISSUED_AT,
    };
    assert_eq!(compute_message_hash(@validation, REF_SIGNER_PUBLIC_KEY), REF_MESSAGE_HASH);
    verify_depositor_validation(
        validation, (REF_R, REF_S), REF_SIGNER_PUBLIC_KEY, REF_ISSUED_AT, MAX_AGE,
    )
        .unwrap();
}
